...
def mhd_to_nrrd(file, totype='nrrd'):
    import SimpleITK as sitk
    annot = sitk.ReadImage(file)
    writer = sitk.ImageFileWriter()
    if file.endswith('.mhd'):
        nrrd = file.replace('.mhd', f'.{totype}')
    else:
        nrrd = file + f'.{totype}'
    writer.SetFileName(nrrd)
    writer.Execute(annot)

def writeadata(adata, outpre='adata.Batch.NPCS',  todense=False):
    import scipy as sc
