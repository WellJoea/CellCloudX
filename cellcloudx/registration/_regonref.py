...
def homoregist(fixed_image,  moving_image, 
    ...
    for imeth, itrans in zip(method, transtype):
        if imeth == 'cv2':
            ...
            rgcv2 = cv2regist(transtype=itrans)
            if pass_error:
                try:
                    rgcv2.regist_transform(fixed_image, moving_image, **sargs)
                except:
                    print('cv2 pass')
            else:
                rgcv2.regist_transform(fixed_image, moving_image, **sargs)

            if hasattr(rgcv2, 'tmat'):
                itmat = itmat @ rgcv2.tmat
                imov = rgcv2.mov_out
                movS.append(imov)
...
def regpair(ifix, imov, method=['cv2', 'itk'], 
                  transtype=['rigid', 'affine'], 
                  inverse=False, **kargs):
    if inverse:
...
def regslice(Fixings, Movings, idxm, windows = 30, top = None, strange=None, 
                 method=['cv2', 'itk'], transtype=['rigist', 'affine'],
                 inverse=True,
                **kargs):
