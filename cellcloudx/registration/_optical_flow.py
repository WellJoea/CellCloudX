...
class optical_flow():
    ...
    def transform(self, moving_img=None,  coords=None, locs=None,  VUs = None, method='linear', **kargs):
        ...
        if locs is not None:
            ...
            mov_locs = self.fieldtransform_point(locs, VUs, method=method)
            self.mov_locs = mov_locs
            return mov_out, mov_locs
        else:
            return mov_out

    def regist_transform(self, fixed_img, moving_img, locs=None, mode='edge', method='linear', order=1, **kargs):
        self.regist(fixed_img, moving_img, **kargs)
        self.transform(mode=mode, locs=locs, order=order, method=method)
