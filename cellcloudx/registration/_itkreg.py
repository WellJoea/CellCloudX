...
class itkregist():
    def __init__(self, transtype=None, resolutions=None, GridSpacing=None,  ParameterFiles=None, verb=False):
    ...
    def regist(self, 
        ...
        if not isinstance(fixed_img, itk.Image):
            fixed_itk = ski.color.rgb2gray(fixed_img) if self.multilayer(fixed_img) and togray else fixed_img
            fixed_itk = itk.GetImageFromArray(np.ascontiguousarray(fixed_itk.astype(np.float32)))
        else:
            fixed_itk = fixed_img

        if not isinstance(moving_img, itk.Image):
            mov_itk = ski.color.rgb2gray(moving_img) if self.multilayer(moving_img) and togray else moving_img
    ...
    def transform(self,  moving_img=None, locs = None, paramsnew=None, 
                  interp_method='skimage', inverse=False, swap_xy=True, 
                  tmat = None, **kargs):
        ...
        if interp_method == 'skimage':
            mov_out = homotransform(moving_img, tmat, inverse=inverse, **kargs)
        else:
            mov_out = itk.transformix_filter( moving_img, 
                                             #fixed_point_set_file_name=loc_file,
                                             #output_directory = './'
                                             transform_parameter_object=paramsnew)
            # mov_locs = itk.transformix_pointset(moving_img, paramsnew,
            #                                     fixed_point_set_file_name=loc_file,
            #                                     output_directory = './')
        self.mov_out = mov_out
        self.mov_locs = None
        if locs is not None:
            mov_locs = homotransform_point(locs, tmat, inverse=inverse,  swap_xy=swap_xy)
