...
def matplotlib_to_plotly(cmap, ncolors=255, decimal=8, cmap_type='rgb'):
    ...
    if cmap_type == 'rgb':
        cmapl = [ [h[i],  f'rgb{tuple(rgbs[i,:3])}'] for i in range(corlist.shape[0])]
    elif cmap_type == 'rgba':
        alpha = np.round(corlist[:,3], decimal)
        cmapl = [ [h[i],  f'rgba{tuple([*rgbs[i,:3], alpha[i]])}'] for i in range(corlist.shape[0])]
    return cmapl
...
def pxsetcolor(color, ctype='discrete', cmap=None):
    ...
    if color is None:
        ...
        if color in dir(px.colors.sequential):
            ...
            return {'color_continuous_scale': cmap}
        else:
