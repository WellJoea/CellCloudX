...
from ..preprocessing._normalize import calculate_qc
# import seaborn as sns
...
def violin_qc(adata, groupby='sampleid', ncols=5,  s=1,
            ...
            columns = ['total_counts', 'n_genes_by_counts', 'pct_counts_mt', 
                       'pct_counts_ribo', 'pct_counts_hb', 'doubletS'], **kargs):
    ...
    COLs = [ i for i in columns if i in adata.obs.columns ]
    if len(COLs)==0:
        print('no column is found in obs.columns.')
        print('run cc.pp.calculate_qc.')
        calculate_qc(adata, inplace=True)
        COLs = [ i for i in columns if i in adata.obs.columns ]
...
def hist_qc(adata, groupby='sampleid', header='before', ncols=3, lcol=2, pform='png',
            hlines = {}, hcolor='grey',
        clims= {'n_genes_by_counts':4000, 'total_counts':5000, 'pct_counts_mt':15, 'pct_counts_ribo':30, 'pct_counts_hb':30},
        columns = ['n_genes_by_counts', 'total_counts', 'pct_counts_mt', 'pct_counts_ribo', 'pct_counts_hb', 'doubletS', 'CC_diff'], 
...
def gene_qc(adata, use_res=None, groupby = 'SID', header='before', show=True, save=None, 
