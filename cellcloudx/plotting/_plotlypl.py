...
def scatters(adata, groupbys, basis='X_umap', use_raw=False,
               cmap = None, matches = None, 
                ...
                figscale=900, werror=30,  width =None, height=None,
...
def pview(*data, X=None, Y=None, Z=None, group=None, group_diff= False, samples=None, random_state=None, **kargs):
    ncells= len(data)
    groupidx = np.repeat(np.arange(ncells), [idata.shape[0] for idata in data] )
    start, sampleidx =0, []
    if not samples is None:
        ...
        for idx in range(ncells):
            ...
            start += isize
        sampleidx = np.concatenate(sampleidx)

    if all([isinstance(idata, pd.DataFrame) for idata in data]):            
        datas = pd.concat(data, axis=0)
        if group is None:
            datas['group'] = groupidx.astype(str)
        else:
            datas['group'] = datas[group]
            if group_diff:
                datas['groupidx'] = groupidx
                datas['group'] = datas['group'].astype(str) + '_' + datas['groupidx'].astype(str)
        if (X is None) and (Y is None) and (Z is None):
            X, Y, Z = datas.columns[:3].tolist()
    else:
        datas = pd.DataFrame(np.vstack(data), columns=list('xyz'))
        datas['group'] = groupidx.astype(str)
        X, Y, Z = list('xyz')
    if len(sampleidx) > 0:
        datas = datas.iloc[sampleidx,:]
    qscatter(datas, X=X, Y=Y, Z=Z, group='group', **kargs)

def pview2d(*data, X=None, Y=None, group=None, group_diff= False, colorlist = [], 
    nrows=None, ncols=None, figscale=600, werror=20, 
    width = None, height=None,  
    facet_col_spacing=None,
    facet_row_spacing=None,
    samples=None, seed=None,  render_mode='auto', 
    save=None, outpre=None, size=None, size_max =20, scale=1, show=True,
    template='none', scene_aspectmode='data',
    y_reverse=False, x_reverse=False,  
    same_scale=True, 
    ...
    **kargs):
    ncells= len(data)
    nrows, ncols = colrows(ncells, ncols=ncols, nrows=nrows)
    width = ncols*figscale+werror  if width is None else width
    height= nrows*figscale if height is None else height
    ...
    if all([isinstance(idata, pd.DataFrame) for idata in data]):            
        ...
        if (X is None) and (Y is None):
            dimdict = dict(zip(list('xy')), datas.columns[:2].tolist())
        else:
            dimdict = dict(zip(list('xy'), [X, Y]))
        datas['cellid'] = datas.index
    else:
        datas = pd.DataFrame(np.concat(data, axis=0), columns=list('xy'))
        datas['group'] = groupidx.astype(str)
        datas['cellid'] = np.hstack([np.arange(idata.shape[0]) for idata in data])
        dimdict = dict(zip(list('xy'), list('xy')))
    
    # color_discrete_sequence = {}
    # if group is None:        
    #     split_color = color_palette(ncells)
    #     dimdict['category_orders'] = {''}

    datas['split'] = groupidx.astype(str)
    if len(sampleidx) > 0:
        datas = datas.iloc[sampleidx,:]

    fig = px.scatter(datas, color="split", 
                    facet_col="split", 
                    facet_col_wrap=ncols,
                    facet_col_spacing=facet_col_spacing,
                    facet_row_spacing=facet_row_spacing,
                    ...
                    # color_discrete_sequence=color_discrete_sequence,
                    # color_continuous_scale = cmap,
                    hover_name="cellid", hover_data=["cellid"],
                    # category_orders={'Type': category_orders},
                    **dimdict, **kargs)

    itemwidth = itemwidth or (30+ncells)
    fig.update_layout(  
        legend=dict( itemsizing = itemsizing, itemwidth=itemwidth, **legdict),
        coloraxis_colorbar=dict(
            # title="",
