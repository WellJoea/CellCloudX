...
class DotPlot():
    def __init__(self, dotcolor='viridis_r', rowcolor=None, colcolor=None, tree_kargs=None,
    ...
    def dot_plot(self, size_df, 
        ...
        cbar.set_label(color_title, size=legend_fs)
    ...
    def clustdot(self,
                ...
                show=True):
        '''
        sig_mean = pd.DataFrame(np.random.rand(20,15), index=np.arange(20), columns=np.arange(15))
        sig_padj = pd.DataFrame(np.random.rand(20,15), index=np.arange(20), columns=np.arange(15))

        cmap = dict(zip('ABCD', cc.pl.random_colors(4)))
        add_col_annot = pd.DataFrame(np.random.choice(list('ABCD'), size=(15,3), replace=True), index = sig_mean.columns).replace(cmap)
        add_row_annot = pd.DataFrame(np.random.choice(list('ABC'), size=(20,2), replace=True), index = sig_mean.index).replace(cmap)
        add_row_annot

        AA = cc.pl.DotPlot()
        AA.clustdot( sig_mean, color_df=sig_padj,
                            #row_threshold=0,
                            #col_threshold=0,
                            ...
                            dotkws={"size_title":'omega weight',
                                        "grid_color":'gray', 'show_yticks':False,
                                        'grid_linestyle' : '-.', 
                                        'grid_linewidth' : 0.15,
                                        'yticklabels_fs':8,
                                        'xticklabels_fs':8,
                        ...
                        #     height_ratios=[1.4,1.5, 1.6, 15]
                            )
        '''
        from matplotlib import gridspec
        size_df = size_df.copy()
        color_df = size_df.copy() if color_df is None else color_df.copy()
        ...
        size_df = size_df.loc[self.row_order, self.col_order]
        color_df= color_df.loc[self.row_order, self.col_order]
        self.dot_plot(size_df, color_df=color_df, ax=self.ax_dotmap, **dotkws)
