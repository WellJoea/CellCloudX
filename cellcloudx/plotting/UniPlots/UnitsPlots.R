...
tzscore <- function(matrx, quant=NULL, thred=NULL, zscore=TRUE){
    #(0.05, 0.95)
    if (!is.null(thred)){
        if (zscore){
            smtx = apply(matrx, 2, function(x){
                vmin = thred[1]
                vmax = thred[2]
                ix = x[((x>=vmin) & (x<=vmax))]

                ixsd = sd(ix)
                ixsd = ifelse(ixsd==0, 1, ixsd)
                (x - mean(ix))/ixsd
            })
        }else{
            smtx = apply(matrx, 2, function(x){
...
scale_data <- function(subobj , slot='data', scale_by='var', 
                       quant=c(0.05, 0.95), thred=NULL, scale.max = 10,
                       scale.method = 'seurat', features=NULL, cells=NULL){
    if (scale.method == 'seurat'){
        subobj <- ScaleData(subobj, features=features)
        sdata <- GetAssayData(object = subobj, slot = 'scale.data')[features, cells, drop = FALSE]
    }else{
        data <- GetAssayData(object = subobj, slot = slot)[features, cells, drop = FALSE]
        if (scale.method == 'zscore'){
            zscore = TRUE
        }else if (scale.method == 'minmax'){
            zscore = FALSE
        }
        if (scale_by =='obs'){
            sdata <- tzscore(data, quant=quant, thred=thred, zscore=zscore)
        }else if (scale_by =='var'){
            sdata <- t(tzscore(t(as.matrix(data)), quant=quant, thred=thred, zscore=zscore))
        }else{
          sdata <- data
        }
    }
    sdata[sdata>scale.max] = scale.max
    sdata
}
...
ComHeatmap <-function(object, 
                      features = NULL, 
                       ...
                       annot.side = NULL,
                        ...
                        annot.fsize = 8, 
                        annot.word_wrap = TRUE,
                        annot.width = unit(8, "cm"),
                        annot.line_space = unit(1.5, "mm"),
                        annot.padding = unit(3, "pt"),
                        annot.new_line = TRUE,
                        annot.by =  "anno_link",
                        annot.text_space = unit(4, "pt"),
                        annot.fontfamily='arial',
    ...
    data <- scale_data(object, slot=slot, scale_by=scale_by, 
                          quant=quant, thred=thred,
                          scale.method = scale.method,
                          features=features, cells=cells)
    #object <- suppressMessages(expr = StashIdent(object = object, save.name = "ident"))
    data <- data[, row.names(groups.use)]
    data <- as.matrix(data)
    disp.max <- disp.max %||% ifelse(test = scale.method != "minmax",  yes = 3,  no = 4)
    disp.min <- disp.min %||% ifelse(test = scale.method != "minmax",  yes = -3, no = 0)
    ...
    top_annotation <- HeatmapAnnotation(df=dplyr::select(groups.use, all_of(group.by)), 
                                  name=group.by,
                                  which="col",
                                  col=colours, 
                                  show_annotation_name=show_top_label,
                                  # annotation_width=unit(rep(col.bar_height, length(group.by)), "cm"), 
                                  # gp = gpar(col_lgp = col_lgp),
                                  simple_anno_size=col.bar_height,
                                  gap=annot_gap)
    ...
    if(!is.null(features.split)){
        ...
        row_anno.1 <- HeatmapAnnotation(df=rowleft,
                              ...
                              which="row",
                              show_legend=FALSE,
                              annotation_name_rot =left_label_rot, 
                              # annotation_width=unit(row.bar_width, "cm"), 
                              simple_anno_size=row.bar_width,
                              gap=annot_gap)
        left_annotation = row_anno.1
        
    }

    if (!is.null(features.select)){
        if (class(features.select) == 'character'){
          features.select <- intersect(features.select, features)
          gene.pos <- which(features %in% features.select)
          gene.labels <- features[gene.pos]
        }else if((class(features.select) == 'list') & (!is.null(features.split))){
          gene.pos <- unlist(lapply( names(features.select), function(x){
                which((features %in% features.select[[x]]) & features.split == x)
          }))
          gene.pos = gene.pos[!is.na(gene.pos)]
          gene.labels <- features[gene.pos]
        }else{
          print('Error: features.select must be a character vector or a list of character vectors.')
          # quit(save = "no", status = 1, runLast = FALSE)
        }

        if(length(gene.pos)==0){
            print('Error: features.select is empty.')
            quit(save = "no", status = 1, runLast = FALSE)
        }

        show_row_names <- FALSE
        if (is.null(select.side)){
            select.side = ifelse(is.null(features.annot), 'right', 'left')
        }
        row_anno.2 <- rowAnnotation( Feautrues = anno_mark(at = gene.pos,
                                                        labels = gene.labels,
                                                        which='row',
                                                        side=select.side,
                                                        lines_gp = grid::gpar(),
        ...
        if(select.side == 'right'){
            right_annotation <- c(row_anno.1, row_anno.2)
        }else{
            left_annotation <- c(row_anno.2, row_anno.1)
        }
    }

    if (!is.null(features.annot)){
        show_row_names <- FALSE
        if (is.null(annot.side)){
            annot.side = 'right'
        }
        row_anno.3 <- rowAnnotation(textbox = anno_textbox(features.split, features.annot, 
                                                            gp = gpar(fontsize = annot.fsize, 
                                                                        fontfamily=annot.fontfamily),  #Arial
                                                            word_wrap = annot.word_wrap,
                                                            # round_corners = TRUE,
                                                            # r = unit(1000, "snpc"),
                                                            max_width = annot.width,
                                                            by = annot.by,
                                                            side=annot.side, 
                                                            text_space = annot.text_space,
                                                            line_space = annot.line_space,
                                                            padding = annot.padding,
                                                            add_new_line = annot.new_line)

        )

        if(annot.side == 'right'){
            if ( (!is.null(features.select)) & select.side == 'right'){
                right_annotation <- c(row_anno.1, row_anno.2, row_anno.3)
            }else{
                right_annotation <- c(row_anno.1, row_anno.3)
            }
        }else{
            if ( (!is.null(features.select)) & select.side == 'left'){
                left_annotation <- c( row_anno.3, row_anno.2, row_anno.1)
            }else{
                right_annotation <- c(row_anno.3, row_anno.1)
            }
        }
    }
    ...
    do_heatmap <- function(){ #disgusting
    ...
    Phmp = draw(Phmp, merge_legend = merge_legend)
    # layer_fun error in raster
    if(!is.null(features.split)){
        if (length(setdiff(unique(features.split),levels(col.split)))==0){
            features.split = factor(features.split,
                                    levels=levels(col.split))
        ...
        min_cycle <- min(row_km, column_km)
        for(i in 1:row_km){
            for(j in 1:column_km){
                if (i %% min_cycle == j %% min_cycle){
                    decorate_heatmap_body(name, row_slice = i, column_slice = j, {
                        grid.rect(gp = grid::gpar(lwd = slwd, col=scol, fill = "transparent"))
                    })
                }
            }
        }
