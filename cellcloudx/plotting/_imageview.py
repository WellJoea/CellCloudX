...
def qview(*arrays, layout=None, 
    ...
    for i in range(ncells):
        ...
        if itp in ['image']:
            ...
            im = ax.scatter(ida[:,0], ida[:,1], s=size, c=color,**kargs)
            ax.set_xlabel('x')
            ax.set_ylabel('y')
...
def drawMatches(pairlist, bgs=None,
    ...
    if show_line and (line_width> 0):
        ...
        rngc = np.random.default_rng(None)
...
def arrow_vector(Y, TY , figsize=(10,10), mutation_scale=5, 
                 arrowstyle='->',
...
def arrow_vector2d(Y, TY , show_points=True, fig=None, ax=None, figsize=(5,5), 
    ...
    Q = ax.quiver(Y[:,0], Y[:,1],  V[:,0], V[:,1], color=C,
    ...
    if show_points:        
        ax.scatter(Y[:,0], Y[:,1], s=size, edgecolors=edgecolors, c='r')
        ax.scatter(TY[:,0], TY[:,1], s=size, edgecolors=edgecolors, c='b')
