...
def scattersplit(adata, groupby, splitby=None, basis='X_umap',
                groups = None, splits=None, titles=None,
                legend_text=False,
                legend_ax = 'merge',
                dpi=500,
                use_raw = False,
    ...
    if (not k_color is None) and len(k_color) > 0:
        assert len(k_color) == k+1

    assert vartype(adata.obs[groupby]) == 'discrete', f'{groupby} must be a discrete variable, found {vartype(adata.obs[groupby])}'
    try:
    ...
    if splitby:
        ...
        if dtype == 'discrete':
            ...
            _sccor = adata.uns.get( splitby+'_colors', [])
            if _sccor is None or len(_sccor) < len(S):
        ...
        _sccor = adata.uns.get( groupby +'_colors', [])
        if _sccor is None or len(_sccor) < len(S):
    ...
    for n,sid in enumerate(G):
        ...
        if titles is None:
            ...
            AX.set_title(titles[n],size=fontsize)
        ...
        ## plot sactters
        if k_term in ['B','both']:
            nii = np.arange(n-k,n+k+1)
        elif k_term in ['R', 'right']:
            nii = np.arange(n, n+k+1)
        elif k_term in ['L', 'left']:
            nii = np.arange(n-k, n+1)
        else:
            raise ValueError(f'k_term must be one of "B", "R", "L", or "both", found {k_term}')

        nii = nii[(nii>=0) & (nii<ncells)]
        texts, x_pos, y_pos = [], [], []

        for ii in nii:
            iisid = G[ii]
            _iiidx = (adata.obs[groupby]==iisid) & (_idxd)
            if np.sum(_iiidx)==0:
                continue

            if splitby is None:
                AX.scatter( locs[:, 0][_iiidx], locs[:, 1][_iiidx], s=size, marker=marker, label=sid, 
                        #    facecolors=_sccor[n], edgecolors=_sccor[n], 
                        edgecolors=edgecolors, 
                        alpha=alpha,
                        rasterized=rasterized,
                        c=_sccor[ii], **kargs)
                ix_pos, iy_pos = pos_center(locs[_iiidx])
                texts.append(iisid)
                x_pos.append(ix_pos)
                y_pos.append(iy_pos)
                ...
                if dtype == 'discrete':
                    for icor, isid in zip(_sccor, S):
                        _iidx = ((adata.obs[groupby]==iisid) & (adata.obs[splitby]==isid))
                        if np.sum(_iidx)>0:
                    ...
                    if idata.shape[0]>0:
                        ...
                        im = AX.scatter(locs[_iiidx][:, 0], locs[_iiidx][:, 1],
                                        s=markers, 
                                        vmin=imin, vmax=imax, 
                        ...
                        ixy_pos = pos_center(locs[_iiidx])
                        x_pos.append(ixy_pos[0])
                        y_pos.append(ixy_pos[1])
                        texts.append(iisid)
    ...
    if (not legend_ax is None) and (type(legend_ax) == str and legend_ax=='merge'):
        if (not splitby is None) and (dtype == 'continuous'):
            ...
            if (splitby is None) and (k>0):
                for iax in axes_list:
                    ihandle, ilabel = iax.get_legend_handles_labels()
                    if len(ihandle) >0:
                        if k_term in ['R', 'right']:
                            handles.append(ihandle[0])
                            labels.append(ilabel[0])
                        else:
                            ik = 0 if len(handles) ==0 else min(k, len(ihandle))
                            handles.append(ihandle[ik])
                            labels.append(ilabel[ik])
            else:
                for iax in axes_list:
                    ihandle, ilabel = iax.get_legend_handles_labels()
                    handles.extend(ihandle)
                    labels.extend(ilabel)
            labelsS = [ ilb for ilb in S if ilb in labels ]
            handles = [dict(zip(labels, handles))[ilb] for ilb in labelsS]
            lrow, lcol = colrows(len(handles), nrows=lnrow, ncols=lncol, soft=False)
            fig.legend(handles, labelsS, loc=legend_loc, bbox_to_anchor=bbox_to_anchor, 
...
def scattersplit0(adata, groupby, splitby=None, basis='X_umap',
