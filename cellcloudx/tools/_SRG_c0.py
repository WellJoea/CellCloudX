...
class SRG(thbase):
    def __init__(self,
        ...
        self.C_gene = C_gene
        self.geneid = th.arange(self.G, device=self.device, dtype=self.dtype)
    ...
    def train(self, 
                ...
                device = None,
                dtype = None,
                simple_edge_h_dim: int = 64,
                simple_edge_hidden: int = 64,
                epochs: int = 500,
                lr: float = 2e-2,
        ...
        if u_enhence:
            ...
            # ✅ StageA_Simple now has edge-gate learned from Exp
            model = StageA_Simple(
                n=n, c=c, n_slices=nB, K=K, R=R,
                mode=mode, B_mode=B_mode,
                use_edge_gate=use_edge_gate,
                edge_h_dim=simple_edge_h_dim,
                edge_hidden=simple_edge_hidden,
                edge_gate_chunk=edge_gate_chunk,
                use_node_reliability=use_node_reliability
            ).to(device)
        ...
        pbar = tqdm(range(epochs), total=epochs,
                    desc=("stageA_enhance" if u_enhence else "stageA_simple"),
                    colour="red", disable=(self.verbose < 1))

        for ep in pbar:
            if u_enhence:
                U, h = model.compute_U(Exp, src, dst, base_w)
                ...
                loss_graph = self.charbonnier_graph_loss(
                    U, h, src, dst, base_w,
                    edge_gate=model.edge_gate if use_edge_gate else None,
                    rho=rho, eps=1e-3,
                    edge_gate_chunk=edge_gate_chunk
                )
            else:
                U = model.U_pos()
                h = model.encode(Exp)  # ✅ Exp -> h
                m, extra = model.forward_obs_mean(Lib, BIv, log_link=self.log_link, eps=self.eps)
                rho = model.rho() if use_node_reliability else None
                loss_graph = self.charbonnier_graph_loss(
                    U, h, src, dst, base_w,
                    edge_gate=model.edge_gate if use_edge_gate else None,
            ...
            loss_graph = float(lambda_graph) * loss_graph
            ...
            if use_node_reliability and rho is not None:
                msg["rho_mean"] = f'{rho.item():.3e}'
        ...
        with th.no_grad():
            if u_enhence:
                U, _h = model.compute_U(Exp, src, dst, base_w)
                Wc = model.W_pos()
                stagea = {
                    "U": U.detach().cpu(),
                    "Wc": Wc.detach().cpu(),
    ...
    def masked_mode_loss(self,  y: th.Tensor, m: th.Tensor, mask: th.Tensor,
        ...
        if mode == "poisson":
            ...
            loss_data = (self.masked_nb_nll(y, m, mask, sigma, eps=self.eps))
        elif mode == "gaussian":
    ...
    # ---------- graph loss ----------
    def charbonnier_graph_loss(self, U: th.Tensor, h: Optional[th.Tensor],
...
class EdgeGate(nn.Module):
    ...
    def __init__(self, dim_h: int, hidden: int = 64,):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dim_h * 2 , hidden), nn.ReLU(),
            nn.Linear(hidden, 1)
        )
...
class StageA_Simple(nn.Module):
    ...
    def encode(self, x_common: th.Tensor) -> Optional[th.Tensor]:
        if not self.use_edge_gate:
            return None
        return self.enc(x_common)
...
class StageA_GNN(nn.Module):
    """
    GNN-enhanced StageA for common genes (algorithmically aligned with dd.py):

    Key ideas borrowed/adapted:
      1) Mask-aware encoder: missing genes should not be treated as true zeros.
      2) Rho (node reliability) is predicted from h (amortized), with an optional anti-collapse regularizer.
      3) Stable message passing: pre-LN by default + residual scaling to avoid U collapse.
      4) Avoid "gate shuts off graph loss" cheating: graph loss can use gate.detach() by default.

    Notes:
      - This class focuses on the algorithm; sparse/dense efficiency is not modified here.
    """
    def __init__(
        self,
        n: int,
        ...
        h_dim: int = 64,
        # mask-aware encoder
        use_mask_in_enc: bool = True,
        enc_ln: bool = True,
        # message passing
        norm_mode: Literal["pre", "post", "none"] = "pre",
        resid_init: float = 0.1,
        # gate / rho
        use_edge_gate: bool = True,
        use_node_reliability: bool = True,
        rho_hidden: int = 64,
        detach_gate_in_graph_loss: bool = True,
        edge_gate_chunk: Optional[int] = None,
    ):
        super().__init__()
        ...
        self.use_mask_in_enc = bool(use_mask_in_enc)
        self.enc_ln = bool(enc_ln)
        self.norm_mode = norm_mode
        self.detach_gate_in_graph_loss = bool(detach_gate_in_graph_loss)
        ...
        # ------------------------------------------------------------------
        # 0) Encoder: x_common -> h0
        #    If use_mask_in_enc:
        #      - normalize by observed gene count to reduce panel-size bias
        #      - concatenate mask so the net can distinguish "missing" vs "true 0"
        # ------------------------------------------------------------------
        enc_in = c * 2 if self.use_mask_in_enc else c
        enc_layers = [
            nn.Linear(enc_in, h_dim),
        ]
        if self.enc_ln:
            enc_layers.append(nn.LayerNorm(h_dim))
        enc_layers += [nn.ReLU(), nn.Linear(h_dim, h_dim)]
        if self.enc_ln:
            enc_layers.append(nn.LayerNorm(h_dim))
        enc_layers += [nn.ReLU()]
        self.enc = nn.Sequential(*enc_layers)

        # Optional: normalize h before feeding to downstream heads (dd.py uses h_norm)
        ...
        # Residual scaling (helps prevent early collapse / explosion)
        self.res_scale = nn.Parameter(th.ones(self.gnn_layers) * float(resid_init))

        if self.norm_mode not in ("pre", "post", "none"):
            raise ValueError(f"norm_mode must be 'pre'|'post'|'none', got {self.norm_mode!r}")
        ...
        # ------------------------------------------------------------------
        # 3) Node reliability rho: amortized from h (dd.py style)
        #    This avoids per-node free parameters (rho_raw) that can "cheat".
        # ------------------------------------------------------------------
        if self.use_node_reliability:
            self.rho_net = nn.Sequential(
                nn.Linear(h_dim, int(rho_hidden)),
                nn.ReLU(),
                nn.Linear(int(rho_hidden), 1),
            )
        else:
            self.rho_net = None
        ...
        if self.u_gen == "gnn":
            ...
            # Stabilize early U scale: avoid softplus(very negative) -> ~0
            nn.init.zeros_(self.toU.weight)
            nn.init.constant_(self.toU.bias, 1.0)  # softplus(1)=1.31
        else:
        ...
        # ------------------------------------------------------------------
        # 5) Gene/global params (same as your original)
        # ------------------------------------------------------------------
        self.W_raw = nn.Parameter(th.randn(c, K) * 0.01)
        self.alpha = nn.Parameter(th.zeros(c))
    ...
    def W_pos(self) -> th.Tensor:
        return F.softplus(self.W_raw) + 1e-8
    ...
    def batch_term(self, sid: th.Tensor) -> th.Tensor:
        """
        Low-rank batch term (mean-centered):
          b_{s,g} = (P_s - mean(P))^T Q_g
        return: (n_cells, c)
        """
        Pm = self.P.mean(dim=0, keepdim=True)          # (1,R)
        return (self.P[sid] - Pm) @ self.Q.t()         # (n,c)

    def rho_from_h(self, h: th.Tensor) -> Optional[th.Tensor]:
        if self.rho_net is None:
            return None
        # dd.py: rho uses normalized h
        hn = self.h_norm(h)
        return th.sigmoid(self.rho_net(hn)).squeeze(-1)
    ...
    def _encode_x(self, x_common: th.Tensor, mask_common: Optional[th.Tensor]) -> th.Tensor:
        """
        Mask-aware encoder input preparation:
          - x_eff = x * mask
          - normalize by observed count to reduce panel-size bias
          - concat mask as features
        """
        if (not self.use_mask_in_enc) or (mask_common is None):
            return x_common

        m = mask_common.to(dtype=x_common.dtype)
        # normalize by observed gene count per cell to reduce "panel size" bias
        k = m.sum(dim=1, keepdim=True).clamp_min(1.0)
        x_eff = (x_common * m) / k * k.mean()  # scale to average observed count
        return th.cat([x_eff, m], dim=1)
    ...
    def gnn_forward(
        ...
        dst: th.Tensor,
        base_w: th.Tensor,
        mask_common: Optional[th.Tensor] = None,
        return_gate: bool = False,
    ) -> Union[th.Tensor, Tuple[th.Tensor, Optional[th.Tensor]]]:
        """
        return:
          - h (n,h_dim) if return_gate=False
          - (h, gate) if return_gate=True (gate is per-edge in [0,1], or None if disabled)
        """
        n = x_common.shape[0]
        x_in = self._encode_x(x_common, mask_common)
        h = self.enc(x_in)
        ...
        for l in range(self.gnn_layers):
            if self.use_edge_gate and (self.edge_gate is not None):
                h_src = h[src]
                h_dst = h[dst]
                gate = self.edge_gate(h_src, h_dst, chunk_size=self.edge_gate_chunk)
                gate_out = gate  # last layer gate (useful for debugging)
                w = base_w.detach() * gate
            else:
                w = base_w.detach()
            ...
            if self.norm_mode == "pre":
                delta = self.upd[l](self.ln[l](neigh))
                h = h + self.res_scale[l] * delta
            elif self.norm_mode == "post":
                delta = self.upd[l](neigh)
                h = self.ln[l](h + self.res_scale[l] * delta)
            else:  # none
                delta = self.upd[l](neigh)
                h = h + self.res_scale[l] * delta
    ...
    def compute_U(
        ...
        """
        Returns:
          U: (n,K) positive factors
          h: (n,h_dim) hidden representation
        """
        h = self.gnn_forward(x_common, src, dst, base_w, mask_common=mask_common, return_gate=False)  # type: ignore
        hn = self.h_norm(h)

        if self.u_gen == "gnn":
            U = F.softplus(self.toU(hn)) + 1e-8
        else:
            U = F.softplus(self.U_base_raw + self.toDelta(hn)) + 1e-8

        return U, h

    # -------------------- graph loss (Charbonnier) --------------------
    def charbonnier_graph_loss(
        self,
        U: th.Tensor,
        h: Optional[th.Tensor],
        src: th.Tensor,
        dst: th.Tensor,
        base_w: th.Tensor,
        eps: float = 1e-3,
        ...
        """
        dd.py-style robust graph loss on U:
          L = mean_e [ w_e * sqrt( mean_k (U_u[k]-U_v[k])^2 + eps^2 ) ]

        Important:
          - We optionally reuse edge gate, BUT by default we detach it here
            to prevent "gate shuts off graph loss" cheating.
        """
        w = base_w.detach()

        if self.use_edge_gate and (self.edge_gate is not None) and (h is not None):
            h_src = h[src]
            h_dst = h[dst]
            gate = self.edge_gate(h_src, h_dst, chunk_size=self.edge_gate_chunk)
            if self.detach_gate_in_graph_loss:
                gate = gate.detach()
            w = w * gate

        rho = self.rho_from_h(h) if (h is not None) else None
        if rho is not None:
            w = w * rho[src].detach() * rho[dst].detach()

        diff2 = (U[src] - U[dst]).pow(2).mean(dim=1)
        return (w * th.sqrt(diff2 + eps * eps)).mean()

    # -------------------- forward_common: same interface --------------------
    def forward_common(
        self,
        U: th.Tensor,
        lib: th.Tensor,
        ...
        """
        Compute distribution parameters for common genes.
        Keeps the semantics consistent with StageA_Simple.

        eta_{i,g} = log(lib_i) + log(U_i^T W_g + eps) + alpha_g + b_{sid(i),g}
        """
        W = self.W_pos()  # (c,K)
        dot = (U @ W.t()).clamp_min(eps)  # (n,c)

        eta = th.log(lib.clamp_min(eps)) + self.alpha[None, :] + th.log(dot)

        if not batch_correct:
            eta = eta + self.batch_term(sid)

        if self.mode in ("poisson", "nb"):
            mu = th.exp(eta.clamp(-20, 20))
        ...
        if self.mode == "gaussian":
            sigma = self.sigma_pos()[None, :].expand_as(m)
            return m, sigma
        else:
            # Student-t (df fixed or learned elsewhere); here reuse sigma_pos as scale
            sigma = self.sigma_pos()[None, :].expand_as(m)
            return m, sigma

    # -------------------- rho regularizer (optional) --------------------
    def rho_regularizer(self, h: th.Tensor) -> th.Tensor:
        """
        Anti-collapse regularizer from dd.py:
          L_rho = mean_i (1 - rho_i)^2
        """
        rho = self.rho_from_h(h)
        if rho is None:
            return th.tensor(0.0, device=h.device, dtype=h.dtype)
        return ((1.0 - rho) ** 2).mean()
