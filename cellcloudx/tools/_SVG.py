...
from ._spatial_edges import coord_edges
...
def coord_edges(coordx, coordy=None,
    ...
    if (coordy is None) and (remove_loop):
        mask = src != dst
        src = src[mask]
        dst = dst[mask]
        dist = dist[mask]
...
def normalizeScalFactor(Locs, D1 = 1., D2 = 3., qauntilescale=True):
    ...
    d = Locs.shape[1]
    if qauntilescale:
        scaleFactor = gmean(np.quantile(Locs,0.975,axis=0)-np.quantile(Locs,0.025,axis=0))/(Locs.shape[0])**(1/d)
    else:
        scaleFactor = gmean(np.max(Locs,axis=0)-np.min(Locs,axis=0))/(Locs.shape[0])**(1/d)
    D1 = D1*scaleFactor
    D2 = D2*scaleFactor
...
def SVG(Exp, Locs, genename = None, filter_cell = 5, filter_gene = 5, D1=1.0, D2=3.0, Ds = None, 
         remove_loop = True, mid_threshold=0.5, fitDist='beta',  kd_method='annoy' ,
    ...
    for iradius in [D1, D2]:
        print(f'*****{iradius}*****')
        src, dst, _=  coord_edges(Locs, radius=float(iradius), n_jobs=n_jobs,
                                 remove_loop=remove_loop, kd_method=kd_method )
        print(f'*****mean edges: {src.shape[0]/Locs.shape[0]}')
...
def findSVG(adata, use_raw=False, basis='spatial', add_key = 'SVGs', n_jobs=1, filter_cell=5, 
             filter_gene=5, scaleFactor=1, mid_threshold=0.5, 
             fitDist='beta', D1=1, D2=3, Ds=None, remove_loop=True,  kd_method='sknn' ,
    ...
    Pdata = SVG(Exp, Locs, genename = geneIndex, 
