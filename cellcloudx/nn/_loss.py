...
def loss_gae(X, X_, H, edge_index, 
    ...
    return (loss_mse(X, X_) + 
            loss_wstructure(H, edge_index, edge_weight =edge_weight, Lambda=Lambda) + 
...
def loss_wstructure(H, edge_index, edge_weight =1, temperature = 1, Lambda=0.5):
    if (not Lambda):
        return 0
    else:
        ...
        loss_ = torch.sum(L_emb * H_emb , -1) * edge_weight
        loss_ = Lambda * torch.nn.LogSigmoid()(loss_/temperature)
        return -loss_.mean()

def loss_(H, edge_index):
    Hnorm = F.normalize(H, dim=1)
    L_emb = Hnorm[edge_index[0]]
    H_emb = Hnorm[edge_index[1]]
    return torch.sum(L_emb * H_emb, -1)

def loss_contrast(H, pos_edge_index, neg_edge_index, edge_weight = 1, num_node=None, temperature=1, Beta=1):
    if not Beta:
        return 0
    N = maybe_num_nodes( pos_edge_index[1], num_node or H.shape[0])
    
    H = F.normalize(H, dim=1)
    pos_simi = torch.sum(H[pos_edge_index[0]] * H[pos_edge_index[1]], -1)
    neg_simi = torch.sum(H[neg_edge_index[0]] * H[neg_edge_index[1]], -1)
    ...
    neg_sum = scatter(denominator, neg_edge_index[1], dim = 0, dim_size=N, reduce='sum') + 1e-16
...
def loss_structure(H, edge_index, Lambda=0.5):
    if (not Lambda):
        ...
        structure = -loss_similar(H, edge_index, normal='logsigmoid', Lambda=Lambda).sum()
        return structure
