...
def calculate_qc(adata: ad.AnnData, percent_top=None, log1p=False, 
    ...
    if inplace:
        sc.pp.calculate_qc_metrics(adata, qc_vars=qc_vars, 
                                percent_top=percent_top, log1p=log1p, inplace=inplace)
    else:
        return sc.pp.calculate_qc_metrics(adata, qc_vars=qc_vars, 
                                percent_top=percent_top, log1p=log1p, inplace=inplace)
...
def Vars(mtx, axis=None):
    ...
    if issparse(mtx):
        mtx_v = mtx.copy()
        mtx_v.data **= 2
        V =  mtx_v.mean(axis) - np.square(mtx.mean(axis))
...
def findHGVsc(adata, batch_key=None, minnbat=None, min_mean=0.0125, min_disp=0.5, n_top_genes=None, 
              max_disp=np.inf, subset=False, flavor='seurat',
    ...
    adata.var.highly_variable &= Vars(adata.X, axis=0)>0
    print(f'Compute HGVs: {adata.var.highly_variable.sum()}')

# @inplaceadata
def dropFuncGene(adata, dropMT=False, inplace=True, dropRibo=False, dropHb=False, verbose=1, **kargs):
    calculate_qc(adata, qc_vars=['mt','ribo','hb'], percent_top=None, log1p=False, inplace=True, **kargs)
    ...
    verbose and print(f'keep HGVs: {adata.var.highly_variable.sum()}')
...
def NormScale(adata, batch_key=None, donormal=True, doscale=True, n_top_genes=None, n_jobs=None,
                ...
                max_value = 10, show_plot=True,
                usehvgs=True, verbose=1, **kwargs):
    adata = adata.copy()
    if savecounts:
    ...
    if 'normal' in adata.uns.keys():
        if adata.uns['normal']['isnormal']:
            donormal = 0
            verbose and print('pass log-normalize.')
        if adata.uns['normal']['isscaled']:
            doscale = 0
            verbose and print('pass scale.')
    else:
        adata.uns['normal'] = {'isnormal': 0, 'isscaled': 0}

    if donormal:
        normalize(adata, inplace=True, dolog=dolog, target_sum=target_sum)
        adata.uns['normal']['isnormal'] = 1

    if local_features is None:
        findHGVsc(adata,  batch_key=batch_key, n_top_genes=n_top_genes, **kwargs)
    else:
        adata.var["highly_variable"] = adata.var_names.isin(local_features)
        verbose and print(f'the number of local features is {len(local_features)}.')

    dropFuncGene(adata, inplace=True, dropMT=dropMT, dropRibo=dropRibo, dropHb=dropHb, 
                 verbose=verbose)
    if (local_features is None) and show_plot:
        sc.pl.highly_variable_genes(adata)

    adata = adata[:, adata.var.highly_variable] if usehvgs else adata
    if doscale:
        Scale(adata, n_jobs=n_jobs, inplace=True,  max_value=max_value, vargress=vargress)
        adata.uns['normal']['isscaled'] = 1
...
def HVG_PR(adata, batch_key=None, n_top_genes =None,  minnbat=2, subset=False,
            ...
            inplace=True, show=True, verbose=1,
