...
def cutreeHybrid(dendro, distM, cutHeight=None, minClusterSize=20, deepSplit=1,
    ...
    if pamRespectsDendro and not respectSmallClusters:
        cf.cw("cutreeHybrid Warning: parameters pamRespectsDendro (TRUE) "
    ...
    cf.ci(f"Going through the merge tree...", 'cyan')
    ...
    if cutHeight is None:
        ...
        cf.ci(f"..cutHeight not given, setting it to {cutHeight :.3f}"
    ...
    if nMergeBelowCut < minClusterSize:
        cf.ci("cutHeight set too low: no merges below the cut.")
