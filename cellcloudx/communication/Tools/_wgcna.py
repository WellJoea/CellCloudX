...
from ..Plots import _wgcnp
from ..Plots._Utilis import labels2colors

from ..Utilis import checkarray, checkAdjMat
import copy
from tqdm import tqdm

from ._cutreeHybrid import cutreeHybrid
...
# public values
networkTypes = ["unsigned", "signed", "signed hybrid", "relu", "leaky_relu2", "elu2", "swish2", "softplus2", "softmax2"]
adjacencyTypes = ["unsigned", "signed", "signed hybrid", "relu", "leaky_relu2", "elu2", "swish2", "softplus2", "softmax2"]
...
class WGCNA():
    def __init__(self, adata, name='WGCNA',
                 TPMcutoff=1,
                 powers=None, RsquaredCut=0.9, MeanCut=100,
                 networkType="signed hybrid", 
                 TOMType="signed",
                 minModuleSize=50, naColor="grey",
                 MEDissThres=0.2,
                 species=None, level='gene', anndata=None, geneExp=None,
                 geneExpPath=None, sep=',', use_block=True,
    ...
    @property
    def power(self):
        return 6 if self._power is None else self._power
    @power.setter
    def power(self, values):
        cf.ci(f"set power as {values}", 'green')
        cf.ci(f"add 'power' to adata.uns...", 'green')
        self.adata.uns['power'] = values
        self._power = values

    def filter_gene_cell(self):
        # Prepare and clean data
        # Remove cols with less than 1 TPM
        self.adata = self.adata[:, (self.adata.X > self.TPMcutoff).any(axis=0)]

        # Check that all genes and samples have sufficiently low numbers of missing values.
        goodGenes, goodSamples, allOK = WGCNA.goodSamplesGenes(self.adata.to_df().T)
        # if not okay
        if not allOK:
            ...
            if np.count_nonzero(goodGenes) > 0:
                ...
                print(f"{OKGREEN}Removing genes: {self.adata.obs.columns[not goodGenes].values}{ENDC}")
            if np.count_nonzero(goodSamples) > 0:
                ...
                print(f"{OKGREEN}Removing samples: {self.adata.obs.index[not goodSamples].values}{ENDC}")
            # Remove the offending genes and samples from the data:
            #self.adata.X = self.adata.X.loc[goodSamples, goodGenes]
            self.adata = self.adata[goodSamples, goodGenes]

    def preprocess(self, adata=None,  cutHeight=float('inf'), save = None):
        """
        removing obvious outlier on genes and samples
        """
        cf.ci("Pre-processing...", 'blue')
        adata = adata if not adata is None else self.adata

        vartree = cl.sci_linkage(adata.X.T, method="average",  metric='euclidean')
        clust = cl.sci_cuttree(vartree, height=cutHeight)
        index = np.where(clust[:,0] == 0)[0]
        cf.ci(f"Keep {index.shape[0]} samples used for download analysis...", 'green')
        _wgcnp.sci_cluster(vartree, color_threshold=cutHeight, labels= adata.var_names)

        self.adata = adata[:, index]
        cf.ci("Done pre-processing..", 'blue')

    def pickpower(self, adata = None, ccorr=None, use_soft=True, cortype='pearson',
                    networkType="unsigned", R2Cut=0.9, MeanCut=100, 
                    Powers=None, nBreaks=10,
                    ccorr_args = {}, **soft_args ):
        cf.ci("pick soft threshold...", 'blue')
        adata = adata if not adata is None else self.adata

        if (ccorr is None) or (not hasattr(self, 'ccorr')) or (self.ccorr is None):
            cf.ci("corrected correlation has not been computed...", 'blue')
            ccorr_args = dict(adata = adata,
                              cortype = cortype,
                              networkType=networkType,
                              **ccorr_args)
            ccorr = self.correctcorr( **ccorr_args )
            cf.ci("add 'ccorr' to adata.uns...", 'green')
            adata.obsp['ccorr'] = ccorr

        if use_soft:
            soft_args = dict( ccorr = ccorr,
                                R2Cut=R2Cut,
                                MeanCut=MeanCut,
                                Powers=Powers,
                                nBreaks=nBreaks,
                                **soft_args)
            self.power, self.sft = self.soft_power(**soft_args)
            _wgcnp.soft_threshold(self.sft, R2Cut=R2Cut)
        else:
            self.power = self.empirical_power(adata.obs.shape[0], acttype=networkType)

    def adjacency(self, adata = None, power=None, networkType="unsigned", 
                  use_ccorr =True, cortype = 'pearson', ccorr_args = {},
                  set_attr = True,
        ...
        adata = adata if not adata is None else self.adata
        weights = self.ScaleWeights(weights, adata.X, scaleByMax=scaleByMax, axis=1 )
        if weights is not None:
            cf.cw("weighted correlation has not been writted...", 'red')
            cf.cw("please use unweighted correlation...", 'red')

        if use_ccorr:
            sadj = adata.obsp['ccorr'] ** power
        else:
            ccorr_args = dict(adata = adata,
                                cortype = cortype,
                                networkType=networkType,
                            **ccorr_args)
            sadj = self.correctcorr( **ccorr_args ) ** power
        cf.ci("add 'ccorr' to adata.uns...", 'green')
        adata.obsp['padjacency'] = sadj

    def TOM(self, adata = None, 
                sadj = None, TOMType="signed",
        ...
        adata = adata if not adata is None else self.adata
        adjMat = np.copy(adata.obsp['padjacency'] if sadj is None else sadj)
        ...
        cf.ci("add 'TOM' to adata.uns...", 'green')
        adata.obsp['TOM'] = tom

    def dyncluster(self, dissTOM = None, **kargs):
        cf.ci("computing dynamic tree based on TOM similarity matrix ...", 'blue')
        # Compressed Row Storage (CRS)
        kwgs = dict(deepSplit=2, 
                    pamRespectsDendro=False,
                    minClusterSize=self.minModuleSize)
        kwgs.update(kargs)

        dissTOM = (1 - self.TOM) if dissTOM is None else dissTOM
        dissT = squareform(dissTOM, checks=False)
        # Call the hierarchical clustering function
        self.geneTree = linkage(dissT, method="average")
        # loc1 loc2 dist counts
        # Module identification using dynamic tree cut:
        self.dynamicMods = cutreeHybrid(dendro=self.geneTree,
                                        distM=pd.DataFrame(dissTOM), **kwgs)
        ngroup = self.dynamicMods['Value'].unique().shape[0]
        cf.ci( f"Identified {ngroup} modules using dynamic tree cut:", 'blue')
        # Convert numeric labels into colors

        self.adata.var['dynamicCluster'] = self.dynamicMods['Value']
        self.adata.var['dynamicColors'] =labels2colors(labels=self.dynamicMods)

    def leiden(self,):
        pass

    def eigengenes(self, **kargs):
        # Calculate eigengenes
        kwgs = dict()
        kwgs.update(kargs)

        MEList = WGCNA.moduleEigengenes(expr=self.adata.to_df(),
                                        colors=self.adata.var['dynamicColors'], **kwgs)
        self.MEList = MEList
        self.MEs = MEList['eigengenes']
        if 'MEgrey' in self.MEs.columns:
            self.MEs.drop(['MEgrey'], axis=1, inplace=True)
        # Calculate dissimilarity of module eigengenes
        MEDiss = pd.DataFrame(1 - np.corrcoef(self.MEs, rowvar=False),
        ...
        plt.figure(figsize=(max(20, round(MEDiss.shape[1] / 20)), 8), facecolor='white')
    ...
    def merge_modules(self, **kargs):
        # Call an automatic merging function
        kwgs = dict(cutHeight=self.MEDissThres)
        kwgs.update(kargs)

        merge = WGCNA.mergeCloseModules(self.adata.to_df(), 
                                        self.adata.var['dynamicColors'],
                                        **kwgs)
        # The merged module colors; Rename to moduleColors
        self.adata.var['moduleColors'] = merge['colors']
        # Construct numerical labels corresponding to the colors
        colorOrder = np.unique(self.adata.var['moduleColors']).tolist()
        self.adata.var['moduleLabels'] = [colorOrder.index(x) if x in colorOrder else None for x in
                                            self.adata.var['moduleColors']]
        # Eigengenes of the new merged modules:
        self.MEs = merge['newMEs']

    def update_eigengenes(self,  **kargs):
    ...
    def findModules(self):
        """
        Clustering genes through original WGCNA pipeline: 
        1.pick soft threshold 
        2.calculating adjacency matrix 
        3.calculating TOM similarity matrix 
        4.cluster genes base of dissTOM 
        5.merge similar cluster dynamically
        """
        print(f"{BOLD}{OKBLUE}Run WGCNA...{ENDC}")

        # Call the network topology analysis function
        self.power, self.sft = WGCNA.pickSoftThreshold(self.adata.to_df(), RsquaredCut=self.RsquaredCut,
        ...
        ax[0].axhline(self.RsquaredCut, color='r')
        ...
        # Set Power
        self.adjacency = WGCNA.adjacency(self.adata.to_df(), power=self.power, adjacencyType=self.networkType)
        ...
        # Convert numeric labels into colors
        self.adata.var['dynamicColors'] = WGCNA.labels2colors(labels=dynamicMods)

        # Calculate eigengenes
        MEList = WGCNA.moduleEigengenes(expr=self.adata.to_df(), colors=self.adata.var['dynamicColors'])
        self.MEList = MEList
        self.MEs = MEList['eigengenes']
        if 'MEgrey' in self.MEs.columns:
            self.MEs.drop(['MEgrey'], axis=1, inplace=True)
        ...
        # Call an automatic merging function
        merge = WGCNA.mergeCloseModules(self.adata.to_df(), self.adata.var['dynamicColors'],
        ...
        # Recalculate MEs with color labels
        self.datME = WGCNA.moduleEigengenes(self.adata.to_df(), self.adata.var['moduleColors'])['eigengenes']
        if 'MEgrey' in self.datME.columns:
            self.datME.drop(['MEgrey'], axis=1, inplace=True)
        self.MEs = WGCNA.orderMEs(self.datME)
    ...
    def analyseWGCNA(self, order=None, geneList=None, show=True):
        ...
        nGenes = self.adata.to_df().shape[1]
        nSamples = self.adata.to_df().shape[0]
        ...
        for label in self.MEs.columns:
            xlabels.append(label[2:].capitalize() + '(' + str(sum(self.adata.var['moduleColors'] == label[2:])) + ')')
        ...
        if self.save:
            ...
            modules = np.unique(self.adata.var['moduleColors']).tolist()
            metadata = self.adata.obs.columns.tolist()
            if order is None:
    ...
    def empirical_power(self, nsample, acttype='unsigned'):
    ...
    def soft_power(cls, ccorr = None,
        ...
        datout = [cls.scaleFreePower(datk[:,i], nBreaks= nBreaks, base=base) 
        ...
        candidate = datout[((datout['SFT.R.sq'] > R2Cut) & (datout['mean(k)'] <= MeanCut))]
        if candidate.shape[0] > 0:
            powerEstimate = candidate['Power'].iloc[0]
            cf.ci(f"Selected power to have scale free network is {powerEstimate}", 'green')
        else:
            powerEstimate = datout.loc[ datout['SFT.R.sq'].idxmax(), 'Power' ]
    ...
    def correctcorr(cls, 
        ...
        if not neighbor is None:
            ...
            adata = adata if not adata is None else cls.adata
            data = checkarray(adata.X).copy()
            if cortype == 'pearson': 
                CORRX = np.corrcoef(data, rowvar=True)
            elif cortype == 'bicor': 
                CORRX = cls.bicor(data, axis=1)
            else:
        ...
        ccorr = cls.activation(CORRX, acttype=networkType)

        if np.count_nonzero(np.isnan(ccorr)) != 0:
            cf.cw(f"Some correlations are NA in matrix", 'red')
        if any(np.diag(ccorr) < 0.999):
            cf.cw(f"Some diagonal correlations are not 1 in matrix", 'red')
    ...
    def bicor(x, y=None,  axis=0):
        if len(x.shape) ==1:
            X = WGCNA.bicor_stand(x)
        else:
            X = np.apply_along_axis(WGCNA.bicor_stand, axis, x)
        if y is None:
            C = np.dot(X.T, X) if axis==0 else np.dot(X, X.T) 
        else:
            if len(y.shape) ==1:
                Y = WGCNA.bicor_stand(y)
            else:
                Y = np.apply_along_axis(WGCNA.bicor_stand, axis, y)
            C = np.dot(X, Y)
        try:
    ...
    @staticmethod
    def scaleFreePower( k, nBreaks=10, base=1):
        """
        ...
        return pd.Series({'SFT.R.sq': model1.rsquared,
                ...
                'max(k)': np.max(k),
                'Density': np.sum(k) / (nGene * (nGene - 1)),
                'Centralization': (np.max(k) - np.mean(k)) * nGene / ((nGene - 1) * (nGene - 2)),
                'Heterogeneity': np.sqrt(nGene * np.sum(k ** 2) / np.sum(k) ** 2 - 1) })
    ...
    # Check that all genes and samples have sufficiently low numbers of missing values.
    @staticmethod
    def goodSamplesGenes(adata, weights=None, minFraction=1 / 2, minNSamples=4, minNGenes=4, tol=None,
        ...
        :param adata:expression data. A data frame in which columns are genes and rows ar samples.
        :type adata: pandas dataframe
        :param weights: optional observation weights in the same format (and dimensions) as adata.
        :type weights: pandas dataframe
        ...
        while changed:
            goodGenes = WGCNA.goodGenesFun(adata, weights, goodSamples, goodGenes, minFraction=minFraction,
                                           minNSamples=minNSamples, minNGenes=minNGenes,
                                           minRelativeWeight=minRelativeWeight,
                                           tol=tol)
            goodSamples = WGCNA.goodSamplesFun(adata, weights, goodSamples, goodGenes, minFraction=minFraction,
                                               minNSamples=minNSamples, minNGenes=minNGenes,
                                               minRelativeWeight=minRelativeWeight)
    ...
    @staticmethod
    def goodGenesFun(adata, weights=None, useSamples=None, useGenes=None, minFraction=1 / 2,
        ...
        :rtype: list of bool
        """
        if not adata.apply(lambda s: pd.to_numeric(s, errors='coerce').notnull().all()).all():
            sys.exit("adata must contain numeric data.")

        weights = WGCNA.checkAndScaleWeights(weights, adata, scaleByMax=True)

        if tol is None:
            tol = 1e-10 * adata.abs().max().max()
        if useGenes is None:
            useGenes = np.repeat(True, adata.shape[0])
        if useSamples is None:
            useSamples = np.repeat(True, adata.shape[1])

        if len(useGenes) != adata.shape[0]:
            sys.exit("Length of nGenes is not compatible with number of columns in adata.")
        if len(useSamples) != adata.shape[1]:
            sys.exit("Length of nSamples is not compatible with number of rows in adata.")

        nSamples = sum(useSamples)
        nGenes = sum(useGenes)
        if weights is None:
            nPresent = adata.loc[useGenes, useSamples].notna().sum(axis=1)
        else:
            nPresent = (adata.loc[useGenes, useSamples].notna() and
        ...
        if weights is None:
            var = np.var(adata.loc[gg, useSamples], axis=1)
            # var = var.sort_index(inplace=True)
        else:
            # need to be fix
            # TODO:colWeightedVars
            var = np.var(adata, w=weights)

        var[np.isnan(var)] = 0
        nNAsGenes = adata.loc[gg, useSamples].isna().sum(axis=1)
    ...
    @staticmethod
    def goodSamplesFun(adata, weights=None, useSamples=None, useGenes=None, minFraction=1 / 2,
        ...
        nGenes = sum(useGenes)
        if weights is None:
            nNAsSamples = np.sum((adata.loc[useGenes, useSamples]).isnull(), axis=0)
        else:
            nNAsSamples = np.sum(np.logical_or(adata[useGenes, useSamples],
    ...
    def pickSoftThreshold(data, dataIsExpr=True, weights=None, RsquaredCut=0.9, MeanCut=100, powerVector=None,
        ...
        :param weights: optional observation weights for data to be used in correlation calculation. A matrix of the same dimensions as adata, containing non-negative weights. Only used with Pearson correlation.
        :type weights: pandas dataframe
        ...
        if powerVector is None:
            powerVector = list(range(1, 11)) + list(range(11, 21, 2))
        ...
        blockRange = np.arange(0, nGenes, blockSize)
        ...
        datout = pd.DataFrame(np.full((len(powerVector), len(colname1)), 666), 
                              columns=colname1, index=powerVector, dtype=object)
        ...
        if use_block:
            while startG < nGenes:
                ...
                if dataIsExpr:
                    ...
                    corx = WGCNA.activation(corx, acttype=networkType)
            ...
            print('without block.')
            corx = np.corrcoef(corOptions.x[0], rowvar=False)
            corx = WGCNA.activation(corx, acttype=networkType)
            if np.count_nonzero(np.isnan(corx)) != 0:
                print(
                    f"{WARNING}Some correlations are NA in block {str(startG)} : {str(endG)}.{ENDC}")
            corx[list(range(corx.shape[0])), list(range(corx.shape[1]))] = 1
            datk = []
            for j in powerVector: # slow
                corxCur = corx.copy() ** j
                datk_j = np.nansum(corxCur, axis=0) - 1
                datk.append(datk_j)
            datk = np.vstack(datk).T

        for n,i in enumerate(powerVector):
            khelp = datk[:, n]
    ...
    def scaleFreeFitIndex(k, nBreaks=10):
        ...
        df['log_p_dk'] = np.log10(df['p_dk'] + 1)
        df['log_p_dk_10'] = np.power(10, df['log_dk'])
    ...
    def getModuleName(self):
        ...
        :rtype: ndarray
        """
        return np.unique(self.adata.obs['moduleColors']).tolist()
    ...
    def getGeneModule(self, moduleName):
        ...
        """
        output = {}
        moduleColors = np.unique(self.adata.obs['moduleColors']).tolist()
        ...
        for color in moduleColors:
            if color in moduleName:
                output[color] = self.adata.obs[self.adata.obs['moduleColors'] == color]
        return output
    ...
    def getModulesGene(self, geneIds):
        ...
        for geneId in geneIds:
            modules.append(self.adata.obs.moduleColors[self.adata.obs.gene_id == geneId])
    ...
    def setMetadataColor(self, col, cmap):
        ...
        if col not in self.adata.obs.columns.tolist():
    ...
    def plotModuleEigenGene(self, moduleName, metadata, show=True):
        ...
        if np.all(moduleName not in modules):
            ...
            heatmap = self.adata[:, self.adata.var['moduleColors'] == moduleName].to_df()
    ...
    def functional_enrichment_analysis(self, type, moduleName, sets=None, p_value=0.05):
        ...
        geneModule = self.adata.var.gene_name[self.adata.var.moduleColors == moduleName]
        ...
        if type in ["GO", "KEGG"]:
            enr = gp.enrichr(gene_list=geneModule,
                             ...
                             cutoff=p_value)
            dotplot(enr.res2d,
                    title=f"Gene ontology in {moduleName} module with {sum(self.adata.var['moduleColors'] == moduleName)} genes",
                    cmap='viridis_r',
                    cutoff=p_value,
    ...
    def updateGeneInfo(self, geneInfo=None, path=None, sep=' ', order=True, level='gene'):
        """
        add/update genes info in adata and geneExpr anndata
        ...
        self.adata = GeneExp.updateGeneInfo(self.adata, geneInfo, path, sep, order, level)

    def updateMetadata(self, metaData=None, path=None, sep=' ', order=True):
        """
        add/update metadata in adata and geneExpr anndata
        ...
        self.adata = GeneExp.updateMetadata(self.adata, metaData, path, sep, order)

    @staticmethod
    def softConnectivity(adata, corOptions=pd.DataFrame(), weights=None, type="unsigned", power=6, blockSize=1500,
                         minNSamples=None):
        """
        ...
        :param adata: a data frame containing the expression data, with rows corresponding to samples and columns to genes.
        :type adata: pandas dataframe
        ...
        :param weights: optional observation weights for adata to be used in correlation calculation. A matrix of the same dimensions as adata, containing non-negative weights. Only used with Pearson correlation.
        :type weights: pandas dataframe
        ...
        while start < nGenes:
            ...
            ad1 = WGCNA.adjacency(adata, weights=weights, selectCols=index1, power=power, adjacencyType=type,
            ...
            NoSamplesAvailable = adata.iloc[:, index1].notna().sum(axis=1)
    ...
    def CalculateSignedKME(self, exprWeights=None, MEWeights=None):
        ...
        :param exprWeights: optional weight matrix of observation weights for adata, of the same dimensions as adata
        ...
        """
        corOptions = []
        if self.datME.shape[0] != self.adata.to_df().shape[0]:
            sys.exit("Number of samples (rows) in 'adata' and 'datME' must be the same.")
        adata = self.adata.to_df()
        datME = self.datME
        if exprWeights is not None:
            exprWeights = WGCNA.checkAndScaleWeights(exprWeights, adata, scaleByMax=False)
        if MEWeights is not None:
            MEWeights = WGCNA.checkAndScaleWeights(exprWeights, datME, scaleByMax=False)

        varianceZeroIndicatoradata = sum(adata.var(axis=0) == 0)
        varianceZeroIndicatordatME = sum(datME.var(axis=0) == 0)
        if varianceZeroIndicatoradata > 0:
            print(f"{WARNING}Some genes are constant. Hint: consider removing constant columns from adata.{ENDC}")
        ...
        output = np.corrcoef(adata.T, datME.T)
        tmp = len(datME.columns)
        col = ["k" + datME.columns[i] for i in range(tmp)]
        self.signedKME = pd.DataFrame(output[0:adata.shape[1], adata.shape[1]:],
                                      index=adata.columns, columns=col)
    ...
    def CoexpressionModulePlot(self, module, numGenes=10, numConnections=100, minTOM=0, filters=None):
        ...
        :type minTOM: float
        :param filters: Dictionary which keys are columns names of adata.var that you want to filter the genes based on it and values are determining which rows you want to keep
        :type filters: dict
        ...
        gene_id = self.adata.var.loc[self.adata.var.moduleColors == module, :]
        ...
        self.TOM.columns = self.adata.to_df().columns
        self.TOM.index = self.adata.to_df().columns
        ...
        nodes = self.adata.var.loc[gene_id,]
