...
from  .pyWGCNA import WGCNA as  PyWN

from ..Config import Config as cf
from ..Tools import Cluster as cl
...
def adjacency(adata, 
    ...
    if add_loop:
        padjacency =  checkdiag(pcorr.copy(), diag=1, fill=True, verbose=False)
    else:
        padjacency =  checkdiag(pcorr.copy(), diag=0, fill=True, verbose=False)
    ...
    padjacency = checkdiag(padjacency, diag=0, fill=True, verbose=False)
    add_neighbors(adata, 
                    key_added = key_adj,
...
def TOM(adata, 
    ...
    if add_loop:
        tom =  checkdiag(tom, diag=1, fill=True, verbose=False)
    else:
        tom =  checkdiag(tom, diag=0, fill=True, verbose=False)
