...
Plotflow.df <-function(GGdf, by1, by2, scaleby=NULL, order1 =NULL, order2= NULL, 
                    angle=45, fontsize=12, axisfize=12, flow.color="darkgrey",
                    flow.alpha=0.7,
                    flow.width=0.4, stratum.width=0.4, stat='counts',  ncol=2, show_label=FALSE,
                    show_flow=TRUE,
                    label_size=8){
    library(ggalluvial)
    library(dplyr)
    ...
    if (show_flow){
        KK <- GG %>% ggplot( aes_string(y = stat, x = by1, stratum = by2, alluvium = by2, fill = by2))+ 
                    geom_lode() + 
                    geom_flow(width = flow.width, color = flow.color, alpha=flow.alpha) +
                    geom_stratum(width = stratum.width)
    }else{
        KK <- GG %>% ggplot( aes_string(y = stat, x = by1, stratum = by2, alluvium = by2, fill = by2))+ 
            geom_lode() + 
            geom_stratum(width = stratum.width)
    ...
    KK <- KK +  guides(fill=guide_legend(ncol=ncol)) + 
                 theme(text = element_text(size=fontsize),
                    axis.text=element_text(size=axisfize),
                       axis.text.x = element_text(angle=angle, hjust=1))
    KK
