...
from ..nn._utilis import seed_torch
from ..nn._utilis import seed_torch, self_loop_check, loadData
from ..nn._loss import loss_recon, loss_structure, loss_mse, loss_gae,loss_contrast,loss_soft_cosin
from ..alignment.ccflib._sswnn import SSWNN, sswnn_match
from ..tools._spatial_edges import spatial_edges

class GRAC():
    def __init__(self, save_model=True, save_latent=True, save_x=True, save_alpha=True, weight_attent=True):
    ...
    def train(self,
        ...
        cellidx = np.argsort(np.concatenate(cellid, axis=0))
        model = GATEConv(
                    [in_dim] + hidden_dims,
                    Heads=Heads,
                    Concats=Concats,
        ...
        pbar = tqdm(range(n_epochs), total=n_epochs, colour='red', desc='self  embedding  ')
        for char in pbar:
            for batch in loader:
                model.train()
                optimizer.zero_grad()
                ...
                H, X_ = model(batch.x, batch.edge_index,
                               edge_weight=(batch.edge_weight if self.weight_attent else None)) #, edge_attr = None
        ...
        with torch.no_grad():
            ...
            Hs = torch.cat(Hs, dim=0)[cellidx,]
            Xs_ = torch.cat(Xs_, dim=0)[cellidx,]
        adata.obsm[add_embed] = Hs.numpy()

        if ( not groupby is None) and (adata.obs[groupby].unique().shape[0]>1) and e_epochs>0:
            ...
            pbar = tqdm(range(e_epochs), total=e_epochs, colour='blue',  desc='cross integrating')
            for epoch in pbar:
        ...
        for batch in loader:
            ...
            H, X_ = model.cpu()(batch_data.x, batch_data.edge_index, 
                                edge_weight=(batch.edge_weight if self.weight_attent else None)) #, edge_attr = None
            Hs.append(H.detach())
            Xs_.append(X_.detach())
        Hs = torch.cat(Hs, dim=0)[cellidx,]
        Xs_ = torch.cat(Xs_, axis=0)[cellidx,]

        self.Lambda = Lambda
        print(f'finished: added to `.obsm["{add_key}"]`')
        print(f'          added to `.layers["{add_key}"]`')
        adata.obsm[add_key] = Hs.numpy()
        adata.layers[add_key] = Xs_.numpy()
