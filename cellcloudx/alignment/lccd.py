...
sys.path.append(r'C:\\Users\\wellj\\Documents\\')
from cellcloud3d.tools._neighbors import Neighbors, mtx_similarity
from cellcloud3d.transform import homotransform_point

class EMRegistration(object):
    def __init__(self, X, Y, X_feat=None, Y_feat=None,
                 gamma=0.9, gamma_clip=[1e-8, 0.95], maxiter=500, beta=0.1,
                 lambd=9e9, theta=0.75, tol=1e-5, tol_r=1e-5, tol_s=1e-8,
                 minp=1e-5, rl_w=1e-5, unif=10, knn=15, normal=True,
                 feat_normal='l2',
                 sigma2=None, sigma2f=None,
        ...
        if self.fE:
            ...
            if feat_normal == 'l2':
                l2x = self.xp.linalg.norm(X_feat, ord=None, axis=1, keepdims=True)
                l2y = self.xp.linalg.norm(Y_feat, ord=None, axis=1, keepdims=True)
                l2x[l2x == 0] = 1
                l2y[l2y == 0] = 1
                ...
                self.X_feat = X_feat / l2x
                self.Y_feat = Y_feat / l2y
            elif feat_normal == 'zscore':
                self.X_feat = self.scale_array(X_feat)
                self.Y_feat = self.scale_array(Y_feat)
    ...
    def lle_W(self, Y=None):
        ...
        W = ssp.csr_array((W.flatten(), (dst, src)), shape=(self.M, self.M), dtype=np.float64)  # CHECK TODO
        W = ssp.eye(self.M) - W
        self.llW = W
    ...
    def feature_P(self):
        cosDist = np.sum(np.square(self.X_feat - self.Y_feat), axis=1)
        self.sigma2f = np.sum(cosDist) / (self.N * self.Df) if self.sigma2f is None else self.sigma2f
        self.Pf = np.exp((0 - cosDist) / (2 * self.sigma2f * 1))
        self.gf = (2 * np.pi * self.sigma2f) ** (self.Df / 2)

        U = self.Pf.mean()
        S2 = self.Pf.var()
        self.cf = self.xp.exp(-(U - 0) ** 2 / (2.0 * S2))
        self.cf = 0

        print('sigma2f....111', self.sigma2f, self.cf, self.gf, U, S2)
        import matplotlib.pyplot as plt
        plt.hist(self.Pf.flatten(), bins=100)
        plt.title('Gmm feature 000000')
        plt.show()

    def feature_P0(self):
        cosDist = np.sum(np.square(self.X_feat - self.Y_feat), axis=1)
        # self.Pf = (2-cosDist)/2
        # Pf =  np.sum(self.X_feat*self.Y_feat, axis=1)
        self.Pf = np.exp(-cosDist)
        ...
        plt.title('Gmm feature 000000')
        plt.show()
        print(self.gf, self.cf, 11111111111111)
...
class affine_reg(EMRegistration):
    ...
    def update_variance(self):
        ...
        # self.gamma = np.sum(self.P.diagonal())/self.M
