...
class gwEMRegistration(thopt, gwEM_core):
    def __init__(self, Xs, Fs=None, 
        ...
        self.init_outlier( w, wa, w_clip=w_clip, use_wm=use_wm, 
                          wma=wma, a0=a0, a1=a1, U=U, use_mrf=use_mrf )

        self.p_version = p_version

        self.tau2_decayto =  self.scalar2vetor(tau2_decayto, L=self.L)  
        self.tau2_decaystop = self.scalar2vetor(tau2_decaystop, L=self.L)  
        ...
        if (self.max_stream is None) or (self.max_stream <= 1) or (not self.cuda_is_available):
            self.optimization = self.optimization_series 
        else:
            self.optimization = self.optimization_series #TODO
    ...
    def multi_feature(self, Fs, Ns=None):
        ...
        if not Fs is None:
            ...
            Fins = [i for i in range(self.L) if not self.is_null(Fs[i])]
            if len(Fins) > 0 :
                Iref = Fins[0]
                Fref = Fs[Iref]

                for l in range(self.L):
                    if self.is_null(Fs[l]):
                        fexist[l] = False
                        Fr.append(None)
                    else:
    ...
    def normal_Xs(self):
        ...
        if self.normal in ['isoscale']: # not recommended
            for l in range(self.L):
            ...
            if self.normal in ['isoscale']:
                if not self.roots is None:
                    Xm[self.roots] = Xm[self.roots].mean(0)
                Xs = self.xp.mean(Xs).expand(self.L).clone()

        elif self.normal in [ 'global', True]: 
            ...
            raise ValueError(f"normal must be in [ 'global', 'isoscale', True, False, 'pass']")
    ...
    def init_omega(self, omega, omega_term='both', root=None):
        self.omega = neighbor_weight(omega, L= self.L, w_term=omega_term, xp =self.xp)
        self.omega = self.to_tensor(self.omega, dtype=self.floatx, device=self.device)
        roots = None

        if root is None:
            idx0 = self.xp.where(self.omega.sum(1) == 0)[0]
            if idx0.shape[0] > 0:
                roots = [ int(i) for i in idx0]
                root = int(idx0[0])
    ...
    def init_outlier(self, w, wa, w_clip=None, use_wm=False, 
                        wma=None, a0=1.0, a1=1.0, U=None, use_mrf=False,
                        mask=None):
        if w is None:
        ...
        if wma is None:
            self.wmas = self.was.clone()
        else:
            self.wmas = self.scalar2matrix(wma, (self.L, self.L), dtype=self.floatx, device=self.device)
        self.use_wm = use_wm
        self.use_mrf = use_mrf & use_wm
        self.wms = self.scalar2matrix(None, (self.L, self.L))

        self.a0 = self.scalar2matrix(a0, (self.L, self.L), dtype=self.floatx, device=self.device)
        self.a1 = self.scalar2matrix(a1, (self.L, self.L), dtype=self.floatx, device=self.device)
        self.U = self.scalar2matrix(U, (self.L, self.L), dtype=self.floatx, device=self.device)

        if mask is not None:
            self.ws  = self.ws*(mask>0)
            self.was = self.was*(mask>0)
            self.wmas = self.wmas*(mask>0)
    ...
    def init_tau2(self, tau2, tau2_clip=None, mask=None):
        if np.any(self.fexist):
                    ...
                    for ll in range(self.L):
                        ...
                        if (self.fexist[l]) and (self.fexist[ll]):
                            for lf in range(self.LF):
                                if self.feat_normal[lf] in ['cos', 'cosine', 'ccosine', 'ccos']:
            ...
            self.tau2_grow = [ 1.0/self.decay_curve(self.tau2_decayto[i], self.tau2_decaystop[i], self.maxiter) for i in range(self.L) ]
            self.DK = [2 if self.feat_normal[lf] in ['cos', 'cosine', 'ccosine', 'ccos'] else self.Ns[lf]
                            for lf in range(self.LF) ]
        else:
            ...
            self.tau2_grow  = self.xp.ones(self.L, self.maxiter)
            self.DK = [1]
    ...
    def default_transparas(self, kargs={}):
        dp= {
            ...
            'D':dict( beta=2.5, alpha=5e2, 
                    ...
                    alpha_decayto = 1.0,  use_p1=False, p1_thred = 0,
    ...
    def init_params(self, ):
        ...
        # if self.use_wm:
        #     self.wms = self.scalar2matrix(None, (self.L, self.L))
        #     for iL, ixs in enumerate(self.OM):
        #         for xl in ixs:
        #             Ui = self.U[iL][xl] or self.Ns[xl]
        #             Mi = self.Ns[iL]
        #             pi0 = self.ws[iL][xl]
        #             iwm = ((1.0 - pi0)*Ui/Mi).expand(Mi)
        #             self.wms[iL][xl] = iwm

        if self.use_mrf: # TODO PASS
            pass
            # self.transparas = {
            #     'beta_mrf': 0.8,
            #     'num_eig': 100,
        ...
        for iarr in ['delta', 'zeta']:
            ivalue = getattr(self, iarr)
            setattr(self, iarr, self.to_tensor(ivalue, dtype=self.floatx, device=self.device))
    ...
    def renew_paras(self):
        self.sigma2[ self.sigma2>0 ] = self.sigma2[ self.sigma2>0 ].max() # trick 

        if self.omega_normal:  #TODO
            self.N_base = self.Ns.mean()/self.Ns
            self.N_base = self.N_base.to(self.device, dtype=self.floatx)
            self.omega = self.omega *self.N_base[None,:]

            omegasum1 = self.omega.sum(1, keepdim=True)
            omegasum1[omegasum1==0] = 1.0
            self.omega = self.omega * 2.0/omegasum1
    ...
    def register(self, callback= None, recompute=True, **kwargs):
        ...
        # self.pwcompute_MRFSmoother() TODO pass
    ...
    def echo_paras(self, paras=None,paras1=None, maxrows=10, ncols = 2):
        if paras is None:
            paras = ['N', 'M', 'D', 'L', 'LF', 'K', 'KF', 'scale_factor', 'use_wm', 'use_mrf', 'use_stable',
