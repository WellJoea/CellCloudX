...
from .operation_expectation import ( features_pdist2, sigma_square, update_pi,
                                     expectation_ko, expectation_xp, expectation_ko1, expectation_xp1, 
                                     expectation_ko_ref, expectation_xp_ref, expectation_ko1_ref, expectation_xp1_ref, )
from .manifold_regularizers import rfdeformable_regularizer, MRFSmoother
...
class rfRegularizer_Dataset(): #Dataset
    def __init__(self, Ys, delta, transparas, transformer,
        ...
        if DL == 0:
            ...
            try: #TODO: parallel
                with tqdm(total=self.L, 
                            desc="rfRegularizer",
                            colour='#AAAAAA', 
                            disable=(verbose==0)) as pbar:
                    for iL in range(self.L):
                        pbar.set_postfix(dict(i=int(iL)))
                        pbar.update()
            
                        for j in range(self.L):
                            if self.transformer[j] == 'D':
                                idr = rfdeformable_regularizer(xp = th, verbose=False, **transparas[j])
                                if iL==j:
                                    idr.compute(Ys[iL][:,:-1], device=device, dtype=dtype) # exclude the z-axis
                                    for ia in ['G', 'U', 'S', 'I', 'J1', 'J2', 'J3', 'E', 'F', ]:
                                        setattr(self, f'E_{iL}_{ia}', getattr(idr, ia))
                                else:
                                    if (self.delta[iL, j] > 0):
                                        H1 = np.array(transparas[iL].get('beta', 0), dtype=np.float64)
                                        H2 = np.array(transparas[j].get('beta', 0), dtype=np.float64)
                                        if (hasattr(self, f'S_{j}_{iL}_US') 
                                            and np.all( H1 == H2 )):
                                            US = getattr(self, f'S_{j}_{iL}_Vh').T
                                            Vh = getattr(self, f'S_{j}_{iL}_US').T
                                        else:
                                        ...
                                        US, Vh = None, None
                                    setattr(self, f'S_{iL}_{j}_US', US)
                                    setattr(self, f'S_{iL}_{j}_Vh', Vh)
            except:
                del idr
                self.__dict__.clear()
                clean_cache()
...
class rfMRFSmoother_Dataset():
    def __init__(self, Ys, use_mrf, transparas,
                  verbose=0,
                  device=None, dtype=th.float32):

        self.L = len(Ys)
        assert self.L == len(transparas) and self.L == len(use_mrf)
                ...
                with tqdm(total=self.L, 
                    ...
                    for iL in range(self.L):
                        ...
                        if use_mrf[iL]:
                            ...
                            imrf = MRFSmoother(
                                    Ys[iL].to(device=device, dtype=dtype), 
                                    beta_mrf = transparas[iL]['beta_mrf'],
                                    alpha_mrf = transparas[iL]['alpha_mrf'],
                                    method = transparas[iL]['mrf_method'],
                                    num_eig   = transparas[iL]['num_eig'],
                                    fast_rank = transparas[iL]['fast_rank'],
                                    knn = transparas[iL]['knn_mrf'],
                                    verbose=0)
                            setattr(self, f'mrf_{iL}', imrf)
                        else:
                            setattr(self, f'mrf_{iL}', None)
            except:
                try:
                    imrf
                except:
...
class rfEM_core(): #th.nn.Module  #TODO: parallel
    ...
    @th.no_grad()
    def optimization_stream(self, max_workers=4): #TODO: parallel
        self.H_upd = True
        n_streams = min(max(1, self.L), max_workers)
        streams = [th.cuda.Stream(device=self.device) for _ in range(n_streams)]

        def _worker_forward(iL, stream):
            with th.cuda.stream(stream):
                if self.transformer[iL] == 'D':
                    ...
                    return {
                        ...
                        'type': 'D',
                        'W': W, 't_c': t_c, 'TY': TY,
                        'sigma2_exp': sigma2_exp, 'sigma2': sigma2, 'iQ': iQ
                    }
                else:
                    ...
                    return {
                        'iL': iL,
                        'type': 'other',
                        'A_ab': A_ab, 's_ab': s_ab, 't_ab': t_ab, 't_c': t_c,
                        'TY': TY, 'sigma2_exp': sigma2_exp, 'sigma2': sigma2, 'iQ': iQ
                    }

        futures = []
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            for iL in range(self.L):
                s = streams[iL % n_streams]
                futures.append(ex.submit(_worker_forward, iL, s))
            results = []
            for f in as_completed(futures):
                results.append(f.result())

        th.cuda.synchronize(device=self.device)
        results_sorted = sorted(results, key=lambda r: r['iL'])

        for r in results_sorted:
            iL = r['iL']
            if r['type'] == 'D':
                W = r['W']; t_c = r['t_c']; TY = r['TY']
                self.H_tmp[iL]['W'] = W
                self.H_tmp[iL]['tc'] = t_c
                self.tmats[iL]['W'] = W
                self.tmats[iL]['tc'] = t_c
            else:
                A_ab = r['A_ab']; s_ab = r['s_ab']; t_ab = r['t_ab']; t_c = r['t_c']; TY = r['TY']
            ...
            self.Qs[iL] = r['iQ']   
            self.sigma2_exp[iL] = r['sigma2_exp']
            self.sigma2[iL] = r['sigma2']
            self.TYs[iL] = TY
    ...
    def forward(self, iL,  H_upd=None ): # 
        ...
        expectation = self.expectation_d if self.use_wm[iL] else self.expectation
        Pt1s, P1s, PXs, Nps = expectation(iL, Xs, TY, XF, YF)
    ...
    def expectation(self, iL, Xs, TY, XF, YF):
        ...
        if not self.use_flat:
            ...
            iexpectation = expectation_ko_ref if use_keops else expectation_xp_ref
        ...
        for xl in range(XL):
            ...
            if (Ni <= min_points) or (Mi <= min_points):
                ...
                gs = Mi/Ni*w[xl]/(1-w[xl]) # Ui = Ni
                tau2_alpha = self.tau2_grow[xl][self.iteration]
    ...
    def expectation_d(self, iL, Xs, TY, XF, YF):
        min_points = 10
        XL, DF, feat_normal = self.XL, self.DK, self.feat_normal
        sigma2, tau2, tau2_auto = self.sigma2[iL], self.tau2[iL], self.tau2_auto[iL]
        ws, was, wms, a0s, a1s, Ua = self.ws[iL], self.was[iL], self.wms[iL], self.a0s[iL], self.a1s[iL], self.Ua[iL]
        w_clip = self.w_clip

        use_keops = self.keops_thr[iL]
        d2f = getattr(self.FDs, f'f{iL}', [None]*XL)
        mrf = getattr(self.MRF, f'mrf_{iL}', None)
        
        if not self.use_flat:
            iexpectation = expectation_ko1 if use_keops else expectation_xp1
        else:
            iexpectation = expectation_ko1_ref if use_keops else expectation_xp1_ref
        ...
        for xl in range(XL):
            ...
            if (Ni <= min_points) or (Mi <= min_points):
                ...
                pi0_new, piu_new = update_pi(iP1, iNp, Ni, Mi, pi0, 
                                          alpha=a1, alpha0=a0, 
                                          U = Ui,
                                          mrf_solver=mrf,
                                          w_clip=w_clip,
                                          xp=self.xp)
                self.ws[ iL][xl] = wa *pi0 + (1-wa)*pi0_new
                self.wms[iL][xl] = wa *piu + (1-wa)*piu_new
    ...
    def maximization(self, *args, transformer='E', **kwargs):
        self.update_sigma2 = self.update_sigma2_ab if self.use_flat else self.update_sigma2_abc
    ...
    @th.no_grad()
    def rf_rigid_maximization(self, iL, Pt1s, P1s, PXs, Nps, Xs, Y, H_upd):
        ...
        TcE  = sum( H_upd[il]['tc']*ik for il, ik in enumerate(eta) if ik >0)
        ...
        # masked delta and zeta
        iY = Y[:,:D-1]
        Jab = Np + zeta.sum()
        # TabE = (zeta @ tabs) #check
        TabE  = sum( H_upd[il].get('t') * ik for il, ik in enumerate(zeta) if ik >0)
        ...
        B += sum( H_upd[il].get('A') * ik for il, ik in enumerate(delta) if ik >0)
    ...
    @th.no_grad()
    def rf_similarity_maximization(self, iL, Pt1s, P1s, PXs, Nps, Xs, Y, H_upd):
        omega, delta, zeta, eta, tc_walk = self.omega[iL], self.delta[iL], self.zeta[iL], self.eta[iL], self.tc_walk
        sigma2, sigma2_exp, sigma2_min = self.sigma2[iL], self.sigma2_exp[iL], self.sigma2_min
        itranspara, itmat = self.transparas[iL], self.tmats[iL]
    ...
    @th.no_grad()
    def rf_affine_maximization(self, iL, Pt1s, P1s, PXs, Nps, Xs, Y, H_upd):
        omega, delta, zeta, eta, tc_walk = self.omega[iL], self.delta[iL], self.zeta[iL], self.eta[iL], self.tc_walk
        sigma2, sigma2_exp, sigma2_min = self.sigma2[iL], self.sigma2_exp[iL], self.sigma2_min
        itranspara, itmat = self.transparas[iL], self.tmats[iL]
    ...
    @th.no_grad()
    def rf_deformable_maximization(self, iL, Pt1s, P1s, PXs, Nps, Xs, Y, H_upd):
        ...
        if itranspara['fast_rank']:
            ...
            for il, idel in enumerate(delta):
                if idel >0:
                    if 'W' in H_upd[il].keys():
                        ilW = H_upd[il]['W']
                        if (iL == il):
                            H_del += idel* (V @ (U.T @ ilW))
                        elif (iL != il):
                            Us = getattr(self.DR, f'S_{iL}_{il}_US')
                            Vh = getattr(self.DR, f'S_{iL}_{il}_Vh')
                            H_del += idel*(Us @ (Vh @ ilW))
                        S_del += idel
            B += H_del
            Pg = (P1 + (S_del + delta1))[:,None] * V
            if (use_p1):
    ...
    @th.no_grad()
    def update_sigma2_abc(self, iL, Pt1s, P1s, Nps, PXs, Xs, TY, Qp = None):
    ...
    @th.no_grad()
    def update_sigma2_ab(self, iL, Pt1s, P1s, Nps, PXs, Xs, TY, Qp = None):
        XL, D, xp =  len(Xs), TY.shape[1], self.xp
        D = D - 1 # ADD
        sigma2_min, sigma2_sync, omega = self.sigma2_min, self.sigma2_sync, self.omega[iL]
        ...
        for xl in range(XL):
            if Xs[xl].shape[0] == 0:
                ...
                trxPx = xp.sum( Pt1s[xl] * xp.sum(Xs[xl][:,:D]  * Xs[xl][:,:D], axis=1) )
                tryPy = xp.sum( P1s[xl]  * xp.sum(TY[:,:D] * TY[:,:D], axis=1))
                trPXY = xp.sum( TY[:,:D] * PXs[xl][:,:D])
                Flj.append(trxPx - 2 * trPXY + tryPy)

        # sigma2
        sigma2_exp =(sum( wlj * flj for wlj, flj in zip(omega, Flj))/
                        sum( wlj * jlj * float(D) for wlj, jlj in zip(omega, Nps)))
        sigma2_exp = xp.clip(sigma2_exp, min=sigma2_min)
        if sigma2_sync:
            sigma2 = sigma2_exp.expand(XL)
        else:
            ...
            for xl in range(XL):
                if Xs[xl].shape[0] > 0:
                    sigma2[xl] = Flj[xl]/Nps[xl]/float(D)
                    if sigma2[xl] < sigma2_min: 
                        sigma2[xl] = xp.clip(sigma_square(Xs[xl][:,:D], TY[:,:D]), min=sigma2_min)
        # Q
        Qp = Qp or 0
        for xl in range(XL):
            iQ  = Flj[xl]/2.0/sigma2[xl] + Nps[xl] * float(D)/2.0 * xp.log(sigma2[xl])
            Qp += omega[xl]*iQ
        return sigma2_exp, sigma2, Qp

    @th.no_grad()
    def update_normalize(self, iLs= None, tmats ={}):
        ...
        if not hasattr(self, 'TY'):
            self.TY  = self.xp.zeros((self.M, self.D), device=device, dtype=dtype)

        if iLs is None:
            iLs = range(self.L)
        elif isinstance(iLs, int):
            iLs = [iLs]
        else:
            iLs = list(iLs)
        ...
        for iL in iLs:
            ...
            iTM = copy.deepcopy(self._tmats[iL]) # deepcopy for tmats
            nTM = tmats.get(iL, {})
            if 'transformer' in nTM:
                itf = nTM['transformer']
            iTM.update(nTM)

            iTM = { k: (v if (v is None or self.is_string_like(v)) else self.to_tensor(v, device=device, dtype=dtype)) for k,v in iTM.items() }

            if itf in 'ESARTILOPN':
                if itf in ['E']:
                    A = iTM['R'] * iTM['s'] * Sr
                    iTM['s'] *= Sr
                elif itf in 'SRTILON':
                ...
                TY = self.to_tensor(self.TYs[iL], device=device, dtype=dtype) * Sx + Tx
                ...
                for ia in ['G', 'U', 'S']:
                    iv =  iTM.get(ia, getattr(self.DR, f'E_{iL}_{ia}', None))
                    if iv is not None:
                        iTM[ia] = iv.to(device, dtype=dtype)
                    else:
                        iTM[ia] = iv

                iTM['Y'] = self.to_tensor(self.Ya[iL][:, :-1], device=device, dtype=dtype)
                iTM['Ym'] = Ty[:-1]
                iTM['Ys'] = Sy
                iTM['Xm'] = Tx[:-1]
                iTM['Xs'] = Sx
            ...
            for i, idel in enumerate(delta):
                if idel >0:
                    if (iL == i):
                        ...
                        Us = getattr(self.DR, f'S_{iL}_{i}_US')
                        Vh = getattr(self.DR, f'S_{iL}_{i}_Vh')
