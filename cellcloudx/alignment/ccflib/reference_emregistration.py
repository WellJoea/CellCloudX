    ...
    def init_transmatrix(self):
        for iL in range(self.L):
            self.tmats[iL] =  self.init_tmat(self.transformer[iL], self.Ms[iL], device=self.device,
                                              dtype=self.floatx)
        self.tcs =  self.xp.hstack([ self.xp.tensor([iixa[:, self.D-1].mean() for iixa in ixa if iixa.shape[0]>0], 
                                                    dtype=self.floatx, device=self.device).mean() 
                                    for ixa in self.Xa])
    ...
    def echo_paras(self, paras=None,paras1=None,  maxbits=60, maxrows=10, ncols = 2):
        if paras is None:
            paras = ['D', 'M', 'L', 'XL', 'Ns', 'FL', 'FDs', 'fexists',  'penal_term', 'w_clip',
                     ...
                     'omega_normal', 'min_points',  'sample', 'sample_growto', 'sample_min', 'sample_stop', 
                      ]
