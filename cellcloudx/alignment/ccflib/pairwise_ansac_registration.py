...
class pwAnsac(thopt):
    ...
    def register(self, 
        ...
        if pairs is None:
            ...
            for iL in range(self.L):
                ...
                pairs.append(ipair.detach().cpu().numpy())
        ...
        inliers, model = adapt_ransac(rpos, qpos, self.transmodel,
                                ...
                                data_weight = (pairs_score if use_weight else None),
        ...
        InLiers = np.zeros(qpos.shape[0], dtype=np.bool_)
        InLiers[inliers] = True

        self.inliers = th.split(th.asarray(InLiers, dtype=th.bool), n_split) 
        self.pairs = pairs
    ...
    def draw_match2D(self,  pairs=None, ins=None, score=None, inliers=None, 
                    line_sample=None,
        ...
        line_sample = list_iter(line_sample)
        line_width = list_iter(line_width)
        line_alpha = list_iter(line_alpha)
        point_size = list_iter(point_size)
        X, TY = self.Xs, self.TY
    
        for idx in ins:
            mridx = pairs[idx][:, 0]
            mqidx = pairs[idx][:, 1]
            iX  = X[idx].detach().cpu().numpy()
            iTY = TY
    
            src_pts0 = iX[mridx][:,:2]
            dst_pts0 = iTY[mqidx][:,:2]

            if inliers is not None:
                src_pts = src_pts0[inliers[idx]]
                dst_pts = dst_pts0[inliers[idx]]
                if score is None:
                    line_widths= None
                else:
                    line_widths = [score[idx], score[idx][inliers[idx]]]
                ...
                drawMatches( (src_pts0, dst_pts0, src_pts, dst_pts), 
                                ...
                                size=point_size[idx],
                                line_sample=line_sample[idx],
                                line_alpha=line_alpha[idx],
                                line_width=line_width[idx],
...
def adapt_ransac(src_pts, dst_pts, model_class,
    ...
    verbose >0 and logger.info(f'Evaluate Transformation: {src_pts.shape[0]} pairs...')
    ...
    verbose > 0 and logger.info(f'Finish ansac registration: {len(Inliers)} pairs, {i} iterations.')
