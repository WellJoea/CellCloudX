...
def kernel_xmm_p(X_emb, Y_emb, pairs, sigma2=None, temp=1,
    ...
    Dist2 = np.square(X_emb[pairs[0]] - Y_emb[pairs[1]])
    Dist2 = np.sum(Dist2, axis=-1)
    sigma2 = np.mean(Dist2)/D if sigma2 is None else sigma2

    P,R = dist2prob(Dist2, sigma2, D, 
                  kernel=kernel, shift=shift, dfs=dfs, temp=temp)
    P = csr_array((P, (pairs[0], pairs[1])), shape=(N, M), dtype=np.float64)
    return P,R, sigma2
...
def kernel_xmm_k(X, Y, sigma2=None, temp=1,
    ...
    P,R = dist2prob(Dist2, sigma2, D, 
                  kernel=kernel, shift=shift, dfs=dfs, temp=temp)
    P = csr_array((P, (src, dst)), shape=(M, N), dtype=np.float64)
    return P,R, sigma2
...
def lle_W(Y, Y_index=None, kw=15, rl_w=None,  method='sknn', eps=np.finfo(np.float64).eps):
    M, D = Y.shape
...
def low_rank_eigen_sp(G, num_eig):
    ...
    S, Q = sci.sparse.linalg.eigs(G.astype(np.float64), k=k)
