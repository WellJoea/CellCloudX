...
def ccd(*args, transformer='affine', **kwargs):
    ...
    if transformer in ['rigid', 'euclidean']:
        ...
        kwargs.update({'isoscale': True})
        return SimilarityRegistration(*args, **kwargs)
    elif transformer in ['similarity']:
        kwargs.update({'isoscale': False})
        return SimilarityRegistration(*args, **kwargs)
        ...
        raise ValueError('transformer must be one of rigid, affine, isosimilarity, similarity, deformable, constraineddeformable')
