...
from .operation_expectation import (centerlize,  center_normalize, normalize, scaling, 
...
class thopt():
    def __init__(self, device=None, device_pre=None, floatx=None, 
        ...
        super().__init__()
        self.xp = th
        self.cuda_is_available = self.xp.cuda.is_available()
        self.device = (self.xp.device('cuda' if self.cuda_is_available else 'cpu') 
                       if device is None else device)
        self.device_pre = (self.device if device_pre is None else device_pre)
    ...
    def is_string_like(self, obj: Any) -> bool:
        if isinstance(obj, str):
            return True
        elif th.is_tensor(obj):
            return False
            ...
            return any([ self.is_string_like(i) for i in obj])
        else:
            return False
    ...
    def scalar2matrix(self, x, L, device=None, dtype=None):
        ...
        if x is None:
            return np.full(L, x).tolist()
        x = self.xp.asarray(x)
        if x.shape == tuple(L):
            x = x
        if x.ndim ==1 and x.shape[0] == L[1]:
            x = x.expand(*L).clone()
        elif x.ndim ==1 and x.shape[0] == L[0]:
            x = x[:, None].expand(*L).clone()
        else:
            x = x.expand(*L).clone()
    ...
    def flatten_list(self, lst, remove_none=True):
        for item in lst:
    ...
    def rle_regex(self, s: str) -> str:
        import re
        return re.sub(r'(.)\1*', lambda m: f'{len(m.group(0))}{m.group(1)}', s)
