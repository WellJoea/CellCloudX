...
from .manifold_regularizers import spsparse_to_thsparse, thsparse_to_spsparse
...
class thopt():
    ...
    def to_tensor(self, X, dtype=None, device=None, todense=False):
        if th.is_tensor(X):
            X = X.clone()
        elif issparse(X):
    ...
    def centerlize(self, X, Xm=None, Xs=None, device=None):
        ...
        X -= Xm
        Xs = self.xp.sqrt(self.xp.sum(self.xp.square(X))/(N*D/2)) if Xs is None else Xs.to(device) # N
        X /= Xs
