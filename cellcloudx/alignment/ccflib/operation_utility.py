...
def split_bins_by_bps(*Xs, bps=None, min_points=100, axis = -1, device=None, zspace=0.0, show=False,  verbose=0, **kargs):
    ...
    LPs, RPs, minpts = [], [], th.zeros(L)
    for xl in range(XL):
        X = th.asarray(Xs[xl]).to(device=device)
        lpos, rpos = get_pos_by_bps(X, bps, axis = axis, zspace=zspace[:, xl], xp=th)
        LPs.append(lpos)
        RPs.append(rpos)
        for iL in range(L):
            l,r = lpos[iL], rpos[iL]
    ...
    for iL in range(L):
        mps = minpts[iL]
        if mps < min_points:
            verbose and print(f"Warning Bin {iL} has less than {min_points} points, will expand the range")
            aps = mps
