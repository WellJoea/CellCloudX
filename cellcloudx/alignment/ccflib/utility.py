...
def split_bins_by_bps(X, bps, X_feats = None, min_points=100, axis = -1, zspace=0.1, xp=th):
    ...
    for l,r in zip(lpos, rpos):
        ...
        if not X_feats is None:
            ixfs = [ ixf[idx] for ixf in X_feats ]
            XFs.append(ixfs)
        else:
            XFs.append(None)
    return (lpos, rpos, Xs, XFs)
