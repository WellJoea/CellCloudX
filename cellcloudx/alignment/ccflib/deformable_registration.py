...
from . import emregistration0, emregistration
...
class DeformableRegistration_1(emregistration.EMRegistration):
    def __init__(self, *args, alpha=None, beta=None, gamma1=None, 
                gamma2=None, kw=15, kl=15,
                #  gamma_decay = 0.99, 
                #  gamma_decay_start = 150, 
                use_fg=False, beta_fg=None, low_rank= 4000,
                fast_low_rank =30000, num_eig=100,
                **kwargs):
        super().__init__(*args, **kwargs)
        ...
        self.use_fast_low_rank = (self.M >= self.fast_low_rank)
        self.low_rank = low_rank
        self.use_low_rank = (self.M >= self.low_rank)
        self.fast_rank = (self.use_low_rank or self.use_fast_low_rank) or (self.gamma1>0 or self.gamma2>0)
        ...
        self.expectation = self.expectation_ko_df if self.use_keops else self.expectation_full_df
        self.pre_compute_paras = self.pre_compute_paras_df
    ...
    def init_L(self, Y=None, kw=15, kl=20, use_unique=False, method='sknn'): #TODO float
        ...
        if self.fast_rank is True:
            logger.info(f'compute G eigendecomposition: use fast_low_rank = {self.use_fast_low_rank}...')
            ...
            if self.use_local:
                ...
                if self.data_level<=1:
                    ...
                    self.Fv = WoodburyB(Av, U, Cv, V, xp=self.xp) #TODO
                    self.Fv1 = WoodburyC(1.0 / self.alpha, U, Cv, V, xp=self.xp)
...
class DeformableRegistration_0(emregistration0.EMRegistration):
    def __init__(self, *args, alpha=None, beta=None, gamma1=None, 
                gamma2=None, kw=15, kl=15,
                #  gamma_decay = 0.99, 
                #  gamma_decay_start = 150, 
