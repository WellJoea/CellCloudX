...
class pwAnsac(thopt):
    ...
    def __init__(self, X, Y, X_feat=None, Y_feat=None, device=None, 
        ...
        self.X = self.to_tensor(X, dtype=self.floatxx, device=self.device)
        self.Y = self.to_tensor(Y, dtype=self.floatxx, device=self.device)
        ...
        if (X_feat is None) or (Y_feat is None):
            ...
            self.X_feat = self.to_tensor(X_feat, dtype=self.floatxx, device=self.device) 
            self.Y_feat = self.to_tensor(Y_feat, dtype=self.floatxx, device=self.device)
            self.fexist = True
    ...
    def register(self, 
        ...
        if pairs is None:
            ...
            mscore = th.ones(pairs.shape[0])

        pairs = th.asarray(pairs, dtype=th.int64, device=self.device).clone()
        mscore = th.asarray(mscore, dtype=self.floatxx, device=self.device).clone()
        ...
        rpos = self.X[mridx].detach().cpu().numpy()
        qpos = self.Y[mqidx].detach().cpu().numpy()
        mscore = mscore.detach().cpu().numpy()
