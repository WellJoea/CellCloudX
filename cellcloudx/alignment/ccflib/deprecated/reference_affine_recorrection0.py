...
class rfAffineRecorrection(thopt):
    def __init__(self, X, Y, X_feat=None, Y_feat=None, 
                 normal='each', 
                 feat_normal='l2', 
                 feat_model = 'gmm',
                 transformer = 'rf-affine',
                 floatx = None,
                 device = None,
    ...
    def maximization(self, iL, Pt1, P1, Np, PX):
        ...
        XabPY = X_hat_ab.T @ self.P.T @ Y_hat
    ...
    def maximization0(self, iL, Pt1, P1, Np, PX):
        iX, iY = self.Xs[iL], self.Ys[iL]

        NpPab = Np + self.sigma2s[iL] * self.Del[iL]
        TabP = self.sigma2s[iL] * (self.delta[iL] @ self.tab)
        NpcPc = Np + self.sigma2s[iL] * self.Phi[iL]
        TcP = self.sigma2s[iL] * (self.phi[iL] @ self.tc)
        ...
        XabPY = PX[:,:self.D-1].T @ Y_hat -self.xp.outer(muXab, P1 @ Y_hat)
        XabPY = X_hat_ab.T @ self.P.T @ Y_hat
        XabPY += self.sigma2s[iL]*self.xp.einsum('l,lij->ij', self.gamma[iL], self.B)

        YPY = (Y_hat.T * P1) @ Y_hat
        YPY.diagonal().add_(self.sigma2s[iL] * self.Gam[iL])
