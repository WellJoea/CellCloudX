...
class ConstrainedDeformableRegistration(DeformableRegistration):
    ...
    def __init__(self, *args, e_alpha = None, source_id = None, target_id= None,
                 score=None, **kwargs):
        ...
        self.P_tilde[self.source_id, self.target_id] = 1 if score is None else score
    ...
    def update_transform(self):
        ...
        if self.low_rank is False:
            ...
            B = self.PX - np.dot(np.diag(self.P1), self.Y) + \
                self.sigma2*(1/self.e_alpha)*(self.PX_tilde - np.dot(np.diag(self.P1_tilde), self.Y)) 
