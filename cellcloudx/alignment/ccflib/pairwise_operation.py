...
from .manifold_regularizers import pwdeformable_regularizer, MRFSmoother
from ...transform import homotransform_points, homotransform_point, ccf_deformable_transform_point, homotransform_mat, ccf_hierarchical_transform_point
...
class pwEM_core():
    def __init__(self):
        ...
        self.expectation_ko1 = expectation_ko1
        self.expectation_xp1 = expectation_xp1
        ...
        self.ccf_hierarchical_transform_point = ccf_hierarchical_transform_point
    ...
    def pwcompute_MRFSmoother(self):
        if self.use_mrf:
            self.mrf = MRFSmoother(
                        self.Y, 
                        beta_mrf = self.transparas['beta_mrf'],
                        alpha_mrf = self.transparas['alpha_mrf'],
                        method = self.transparas['mrf_method'],
                        num_eig   = self.transparas['num_eig'],
                        fast_rank = self.transparas['fast_rank'],
                        knn = self.transparas['knn_mrf'],
                        verbose = self.verbose>1)
        else:
            self.mrf = None

    def pwcompute_regularizer(self):
        if self.transformer in 'DH':
            try:
                self.DR = self.pwdeformable_regularizer(**self.transparas, xp = self.xp, verbose=self.verbose>1)
    ...
    def pwexpectation(self):
        ...
        for iL in range(self.L):
            ...
            use_samplex = bool(self.sample[iL]) and (self.iteration < self.sample_stop)
            if use_samplex:
            ...
            # Ui = self.xp.prod(iX.max(dim=0)[0]-iX.min(dim=0)[0]) # TODO U
            # print(Ui, Ni, wi)
            gs = Mi/Ni*wi/(1-wi)
    ...
    @th.no_grad()
    def pwexpectation_d(self):
        Pt1s, P1s,  PXs, Nps = [], [], [], []
        for iL in range(self.L):
            iX, iTY, fexist = self.Xs[iL], self.TY, self.fexists[iL]
            xFa, yFa, d2f, pi0, wa, wma, w_clip = self.XF[iL], self.YF[iL], self.d2f[iL], self.ws[iL], self.wa, self.wma, self.w_clip
            a0, a1, U, piu, mrf = self.a0[iL], self.a1[iL], self.U[iL], self.wms[iL], self.mrf
            ...
            iexpectation = expectation_ko1 if self.use_keops[iL] else expectation_xp1

            iPt1, iP1, iPX, iNp, tau2s = iexpectation( 
                    iX, iTY, self.sigma2[iL], piu, pi0,
                    d2f=d2f, 
                    tau2=self.tau2[iL], 
                    tau2_auto=self.tau2_auto[iL], 
                    tau2_alpha=self.tau2_grow[iL][self.iteration], 
                    DF= self.DK[iL],
                    feat_normal=self.feat_normal[iL],
                    XF=xFa, YF=yFa, 
            ...
            # self.ws[ iL] = pi0_new
            # self.wms[iL] = piu_new
            self.ws[ iL] = wa *pi0 + (1-wa)*pi0_new
            self.wms[iL] = wma *piu + (1-wma)*piu_new
    ...
    def optimization(self):
        qprev = self.Q
        Pt1s, P1s, PXs, Nps = self.pwexpectation_d() if self.use_wm else self.pwexpectation() 
    ...
    def maximization(self, *args, transformer='E', **kwargs):
        if transformer == 'E':
            ...
            self.pw_hierarchical_maximization(*args, **kwargs)
        else:
    ...
    def pw_affine_maximization(self, Pt1s, P1s, Nps, PXs, Y, Xs):
        ...
        if itranspara['delta'] > 0:
            ...
            try:
                ...
                delta = 1e-2
                YPYh.diagonal().add_(delta*sigma2)
                A = (B + delta*sigma2*itmat['A']) @ xp.linalg.inv(YPYh)
    ...
    def pw_projective_maximization(self, Pt1s, P1s, Nps, PXs, Y, Xs):
        ...
        def closure():
            ...
            # self.logsigma2.data = self.xp.nan_to_num(self.logsigma2, nan=0.0, posinf=12.0, neginf=-12)
            ...
            # reg_B = 0.01 * self.xp.sum(self.B ** 2)
            # Q += reg_B

            Q.backward(retain_graph=True)
            # self.xp.nn.utils.clip_grad_norm_([self.A, self.B], max_norm=10.0)
            # self.xp.nn.utils.clip_grad_norm_([self.t, self.logsigma2],  max_norm=100.0)
            return Q
    ...
    @th.no_grad()
    def pw_hierarchical_maximization(self, Pt1s, P1s, Nps, PXs, Y, Xs):
        xp, D, L = self.xp, self.D, self.L
        sigma2_exp, itranspara, itmat = self.sigma2_exp, self.transparas, self.tmats
        sigma2, omega, sigma2_min = self.sigma2, self.omega, self.sigma2_min
        Yh = self.Yh
        ...
        # W
        wsa = sum( omega[iL] * float(alpha[iL]) * mu for iL in range(L) )
        B = PX @ (s*R) - (s**2) * P1[:,None] * Y - (P1[:, None] * (s* t @ R))
        if delta>0:
            P1_hat = P1 + delta #  delta * \bar{P_1}
        else:
            P1_hat = P1
        ...
        if itranspara['fast_rank']:
            ...
            if (use_p1):
                P1s_thred = [ xp.where(iP1 < ipth, 0.0, iP1) for iP1, ipth in zip(P1s, p1_thred) ]
                if any(gamma1 > 0):
                    P2 = sum( (w * float(ig1) * nu1 * (s**2)) * iP1 for w, iP1, ig1 in zip(omega,  P1s_thred, gamma1) )
                    P2 = self.DR.E.T * P2[None,:]
                    B  -= P2 @ self.DR.J3
                    Pg += P2 @ self.DR.J1
                if any(gamma2 > 0):
                ...
                if any(gamma1 > 0):
                    P2 = sum( (w * float(ig1) * nu1 * (s**2)) for w, ig1 in zip(omega, gamma1) )
                    B  -= P2 * self.DR.J3
                    Pg += P2 * self.DR.J1
                if any(gamma2 > 0):
                    P3 = sum( (w * float(ig2) * nu2)  for w, ig2 in zip(omega, gamma2) )
    ...
    def update_normalize(self):
        ...
        # T =  \frac{X_s}{Y_s} Y sR^T +  X_s V sR^T + 1_m (X_m^T + (X_s t)^T- \frac{X_s}{Y_s} Y_m^T sR^T )  
        # T =  \frac{X_s}{Y_s} Y A^T +  X_s V A^T + 1_m (X_m^T + (X_s t)^T- \frac{X_s}{Y_s} Y_m^T A^T )  
        if not itf in 'ESRTILOAPDNH':
            raise ValueError(f'Invalid transformation type {itf}, should be in "ESRTILOAPDNH"')
    ...
    def transform_point(self, Yr, use_keops=True, **kargs):
        ...
        elif self.tmats['transformer']  == 'H':
            return self.ccf_hierarchical_transform_point(Yr, **self.tmats, 
                                                        use_keops=use_keops, **kargs)
        else:
