...
class Mesh_correction:
    ...
    def extract_contours(
        ...
        pbar = tqdm(range(self.n_slices), total=self.n_slices, colour='black', desc=progress_name)
        for model_index in pbar:
    ...
    def run_discrete_optimization(
        ...
        print(f"Run discrete optimization on {len(self.contours_subsample)} contours")
        pbar = tqdm(range(self.max_iter), total=self.max_iter, colour='black', desc=progress_name)
        for i in pbar:
            cur_loss, cur_transformation = self.discrete_optimization_step()

            if self.verbose:
                print(f"Iteration {i+1}/{self.max_iter}, current loss: {cur_loss}")
        ...
        print(f"Optimization finished. Best loss: {self.best_loss}")
