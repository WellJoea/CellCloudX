...
class EMRegistration(thopt):
    def __init__(self, X, Y, X_feat=None, Y_feat=None, 
        ...
        self.w_clip = w_clip
        self.w = np.sqrt(((self.N-self.M)/max(self.N, self.M))**2) if w is None else w
        self.w = self.to_tensor(np.clip(self.w, *self.w_clip), 
        ...
        if self.fexist:
            if self.pre_cumpute_fp :
                ...
                if self.reg_core in ['rigid', 'affine']:
                    self.cdff = self.xp.sum(self.Pf, 0) #/ K
                else:
                    self.cdff = self.xp.sum(self.Pf, 0) #/ K
                self.cf = 0
    ...
    def echo_paras(self, paras=None, maxrows=10, ncols = 2):
        ...
        dvinfo = [self.get_memory(idv) for idv in [self.device, self.device_pre]]
    ...
    def register(self, callback= None, **kwargs):
        ...
        if self.verbose:
            ...
            for i in pbar:
                if (not self.sigma2_step is None) and (i % self.sigma2_step == 0):
    ...
    def update_P(self, X, Y):
        ...
        if self.fexist:
            # cdfs = cdfs * Pa.to_dense()
            # gs = gs / self.gf*self.M
            # cs = cs + self.cf*self.w
            P.multiply_(self.Pf) #* self.M
        cdfs = self.xp.sum(P, 0).to_dense()
        # print(cdfs, cdfs.max(), gs * cs, )
        Cv = 1.0/(cdfs + self.xp.max(gs * cs , self.eps))
        return P, Cv
