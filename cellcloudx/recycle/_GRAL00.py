...
from ._utilis import seed_torch, self_loop_check, loadData
from ._nnalign  import aligner
from ..transform import homotransform_point, homotransform, rescale_tmat

from tqdm import tqdm
torch.set_default_dtype(torch.float32)

class GRAL():
    def __init__(self, save_model=True, save_latent=True, save_x=True, weight_attent=True,
                  save_alpha=True):
        ...
        for char in pbar:
            for batch in loader:
                ...
                H, X_ = model(batch.x, batch.edge_index, 
                            #   edge_weight=(batch.edge_weight if self.weight_attent else None),
                              edge_attr = None)
    ...
    @torch.no_grad()
    def infer(self, adata, groupby = None, basis='spatial', add_align = 'align', img_key="hires", img_add_key = 'tres',
             add_self_loops = True, validate = True, infer_latent=True, infer_alignment=True,
             model = None, device='cpu', verbose=2):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') if device is None else torch.device(device)
        ...
        if infer_latent:
            ...
            for batch in loader:
                ...
                H, X_ = model(batch_data.x, batch_data.edge_index)
        ...
        if infer_alignment and (not groupby is None):
            ...
            adata.obsm[add_align] = np.zeros_like(adata.obsm[basis])
            ...
            for igroup in order:
                ...
                adata.obsm[add_align][idx] = inew_pos
