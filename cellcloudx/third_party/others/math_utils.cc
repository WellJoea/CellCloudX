...
Matrix cellcloudx::kernelBase(const Matrix& x, const Matrix& y, const func_type& fn) {
...
Matrix cellcloudx::squaredKernel(const Matrix& x, const Matrix& y) { return kernelBase(x, y); }

Matrix cellcloudx::rbfKernel(const Matrix& x, const Matrix& y, Float beta) {
    return kernelBase(x, y, [&beta](const Vector& diff2) { return (-diff2 / (2.0 * beta)).array().exp(); });
}

Matrix cellcloudx::tpsKernel2d(const Matrix& x, const Matrix& y) {
...
Matrix cellcloudx::tpsKernel3d(const Matrix& x, const Matrix& y) {
    return kernelBase(x, y, [](const Vector& diff2) { return -diff2.array().sqrt(); });
}

Matrix cellcloudx::inverseMultiQuadricKernel(const Matrix& x, const Matrix& y, Float c) {
